...
func GetYucoreMediaAdapterInfo() YucoreMediaAdapterInfo {
	...
	return YucoreMediaAdapterInfo{
		...
		Status:                      status,
		Message:                     message,
		SupportsImage:               config.Adapter == YucoreMediaAdapterMock || (configured && (config.Adapter == YucoreMediaAdapterOpenAICompatible || config.Adapter == YucoreMediaAdapterUAGProxy)),
		SupportsVideo:               config.Adapter == YucoreMediaAdapterMock || (configured && config.Adapter == YucoreMediaAdapterUAGProxy),
...
func resolveYucoreMediaTaskAdapter(task *YucoreMediaTask) (string, error) {
	...
	case YucoreMediaAdapterOpenAICompatible:
		if task.Kind != "image" {
			return "", errors.New("openai-compatible YuCore media adapter currently supports image tasks only")
		}
	...
	default:
		return "", errors.New("YUCORE_MEDIA_ADAPTER must be mock, openai-compatible, or uag-proxy")
	}
}
...
func YucoreMediaTaskAssets(task *YucoreMediaTask) []YucoreMediaAsset {
	...
	if err := json.Unmarshal([]byte(task.Assets), &assets); err != nil {
...
func yucoreMediaReferenceAssets(task *YucoreMediaTask) []string {
	...
	var inputs []any
	if task.Inputs != "" {
		_ = json.Unmarshal([]byte(task.Inputs), &inputs)
	}
...
func runOpenAICompatibleYucoreImageTask(task *YucoreMediaTask, config yucoreMediaAdapterConfig) error {
	endpoint, err := yucoreMediaEndpoint(config.BaseURL)
	if err != nil {
		return err
	}
	...
	if size := normalizeYucoreMediaImageSize(task.Size); size != "" {
		body["size"] = size
	}
	if task.Quality != "" && !strings.EqualFold(task.Quality, "auto") {
		body["quality"] = task.Quality
	}
	if task.Format != "" && !strings.EqualFold(task.Format, "url") && !strings.EqualFold(task.Format, "b64_json") {
		body["output_format"] = strings.TrimPrefix(task.Format, "image/")
	}
	if task.AspectRatio != "" && !strings.EqualFold(task.AspectRatio, "auto") {
		body["aspect_ratio"] = task.AspectRatio
	}
	if responseFormat := strings.TrimSpace(common.GetEnvOrDefaultString("YUCORE_MEDIA_RESPONSE_FORMAT", "")); responseFormat != "" {
...
func RunYucoreMediaTaskWithHeaders(taskId int, upstreamHeaders YucoreMediaUAGProxyHeaders) {
	...
	config := getYucoreMediaAdapterConfig()
	adapter := yucoreMediaTaskAdapter(&task)
	if adapter != YucoreMediaAdapterOpenAICompatible && adapter != YucoreMediaAdapterUAGProxy {
		return
	}
	task.Progress = 24
	task.UpdatedTime = common.GetTimestamp()
	...
	switch adapter {
	case YucoreMediaAdapterOpenAICompatible:
		err = runOpenAICompatibleYucoreImageTask(&task, config)
	case YucoreMediaAdapterUAGProxy:
