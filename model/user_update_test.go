...
func TestInsertWithTxRejectsDuplicateEmailWithoutUniqueIndex(t *testing.T) {
	setupUserUpdateTestState(t)

	require.NoError(t, DB.Create(&User{
		Username: "existing-transactional",
		Password: "old-password",
	...
	user := &User{
		Username: "oauth-transactional-user",
		Email:    "TRANSACTIONAL@example.com",
		Role:     common.RoleCommonUser,
		Status:   common.UserStatusEnabled,
	}
...
func TestValidateAndFillRejectsPasswordlessUser(t *testing.T) {
	...
	require.NoError(t, DB.Create(&User{
		Username: "passwordless-login-user",
		Password: "",
		Status:   common.UserStatusEnabled,
	}).Error)
	...
	var stored User
	require.NoError(t, DB.Where("username = ?", "passwordless-login-user").First(&stored).Error)
	assert.Empty(t, stored.Password)
}
