...
func TestGetUnrefundedFailedTasksFiltersLegacyAndLimits(t *testing.T) {
	truncateTables(t)
	...
	assert.Empty(t, GetUnrefundedFailedTasks(time.Now().Add(-time.Second).Unix(), 10))
...
func TestRestoreQuotaAfterFailedRefundOnlyRestoresClaimedMarker(t *testing.T) {
	truncateTables(t)
...
func TestHasTaskPollingWorkIncludesOnlyRefundableFailedTasks(t *testing.T) {
	truncateTables(t)
