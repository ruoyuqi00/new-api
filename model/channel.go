...
func BatchDeleteChannels(ids []int) (int64, error) {
	...
	for _, chunk := range lo.Chunk(ids, 200) {
		...
		if err := tx.Where("channel_id in (?)", chunk).Delete(&ChannelAccountPoolBinding{}).Error; err != nil {
			tx.Rollback()
			return 0, err
		}
	}
...
func (channel *Channel) Delete() error {
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		return tx.Where("channel_id = ?", channel.Id).Delete(&ChannelAccountPoolBinding{}).Error
	})
}
...
func DeleteDisabledChannel() (int64, error) {
	...
	deletedCount, err := BatchDeleteChannels(ids)
	if err != nil {
		return 0, err
	}
	return deletedCount, nil
...
func (channel *Channel) ValidateSettings() error {
	...
	if err := ValidateImageCapabilitySettings(*channelOtherSettings); err != nil {
		return err
	}
	...
	if channel.Type == constant.ChannelTypeAdvancedCustom &&
		...
		if _, ok := channelOtherSettings.AdvancedCustom.ModelListRoute(); !ok {
			return fmt.Errorf("advanced_custom requires an explicit %s route when upstream model checks are enabled", dto.AdvancedCustomModelListPath)
		}
	}
	return nil
}
