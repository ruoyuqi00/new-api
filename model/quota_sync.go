...
func CreateQuotaSyncCode(userId int) (string, *QuotaSyncCode, error) {
	if userId <= 0 {
		return "", nil, errors.New("invalid user id")
	}
	secret, err := common.GenerateRandomCharsKey(32)
	if err != nil {
		return "", nil, err
	}
	plain := "NQS-" + secret
	...
	row := &QuotaSyncCode{
		UserId:      userId,
		CodeHash:    quotaSyncHash(plain),
		Status:      QuotaSyncCodeStatusActive,
		CreatedTime: now,
		ExpiresTime: now + 10*60,
	}
