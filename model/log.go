...
type Log struct {
	...
	ActualResponseModel *string `json:"actual_response_model,omitempty" gorm:"type:varchar(100)"`
...
func RecordConsumeLogWithError(c *gin.Context, userId int, params RecordConsumeLogParams) error {
	...
	var actualResponseModel *string
	if value := strings.TrimSpace(params.ActualResponseModel); value != "" {
		actualResponseModel = &value
	}
	// 判断是否需要记录 IP
	needRecordIp := false
	if settingMap, err := GetUserSetting(userId, false); err == nil {
