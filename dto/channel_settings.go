...
type ChannelOtherSettings struct {
	...
	IgnoreClientMaxOutputTokens           bool                            `json:"ignore_client_max_output_tokens,omitempty"` // 是否忽略客户端 max_output_tokens，避免推理预算过早耗尽
	...
	ImageModelCapabilities                map[string]ImageModelCapability `json:"image_model_capabilities,omitempty"`
}
...
// AdvancedCustomModelListPath is the optional OpenAI-compatible upstream
// discovery route. It is used only by channel administration and scheduled
// model checks, never by customer relay selection.
...
// ModelListRoute returns only an explicitly configured discovery route. A
// template that can happen to match this path is not a model discovery route.
	...
	for i := range c.Routes {
		...
		if route.IncomingPath == AdvancedCustomModelListPath {
			...
			if strings.Contains(upstreamPath, advancedCustomModelPlaceholder) {
				return fmt.Errorf("advanced_custom.advanced_routes[%d].upstream_path must not contain {model} for /v1/models", i)
			}
		}
