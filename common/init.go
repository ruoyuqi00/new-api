...
func InitEnv() {
	...
	if SessionCookieSecure {
		if len(sessionSecret) < 32 {
			log.Fatal("SESSION_COOKIE_SECURE=true requires SESSION_SECRET to be at least 32 bytes")
		}
		var distinctBytes [256]bool
		distinctByteCount := 0
		for index := range sessionSecret {
			value := sessionSecret[index]
			if distinctBytes[value] {
				continue
			}
			distinctBytes[value] = true
			distinctByteCount++
		}
		if distinctByteCount < 8 {
			log.Fatal("SESSION_SECRET must not be an obvious low-diversity repeated value when SESSION_COOKIE_SECURE=true")
		}
	}
	if sessionSecret != "" {
...
func initConstantEnv() {
	...
	constant.StreamFirstTokenTimeoutSeconds = GetEnvOrDefault("STREAM_FIRST_TOKEN_TIMEOUT_SECONDS", 120)
