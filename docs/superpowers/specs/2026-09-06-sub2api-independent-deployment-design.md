...
- `sub2.yuaiapi.com` is proxied through Cloudflare. It currently returns HTTP
  `525`, so the origin TLS virtual host is not yet working.
- SSH TCP connectivity to `154.219.122.197:22` succeeds, but the server closes
  the connection before presenting an SSH banner. No remote command was
  executed during design. Live container, port, proxy, and network state must
  therefore be audited before any production mutation.
...
- Shared YuAPI data-plane network: external network `yuapi-backplane`
...
network and `yuapi-backplane`.

The YuAPI container joins `yuapi-backplane` but never joins the private
PostgreSQL/Redis network. This gives YuAPI access to `sub2api-internal:8080`
without granting access to Sub2API's data stores.
...
allowed traffic to `127.0.0.1:18473`.
...
other containers. Docker service discovery scopes the name to `yuapi-backplane`.

YuAPI should use its dedicated Sub2API channel type because that adapter
preserves the OpenAI, Responses, Claude, and Gemini protocol paths. Execution
first audits the live YuAPI image and repository baseline:

- If the dedicated adapter exists, use it without a code deployment.
- If it does not exist, port only the current official minimal Sub2API channel
  adapter, registration constants, frontend channel option, i18n entries, and
  behavioral tests. Do not import the historical multi-feature commit that also
  changed billing, alpha search, and unrelated relay behavior.
...
| Unrelated container without `yuapi-backplane` | Cannot resolve/reach Sub2API alias |
| New Sub2API API key with empty pool | No successful model relay; no YuAPI routing enabled |
...
4. Disconnect YuAPI from `yuapi-backplane` only if the network was introduced
   solely for this deployment and no other approved service uses it.
...
- the dedicated YuAPI channel adapter would require unrelated billing or relay
  changes.
