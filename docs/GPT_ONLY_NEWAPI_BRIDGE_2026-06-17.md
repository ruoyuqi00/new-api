...
- The current GPT account pool is the `GPT5.5` group, group id `8`, platform `openai`.
...
- Treat `GPT5.5` as the current GPT supply group.
...
| Sub2API group | `GPT5.5` | Internal/supply | Current GPT account pool. Keep name for now to avoid breaking existing references. |
...
1. In Sub2API, keep `GPT5.5` as the GPT supply group.
...
3. In Sub2API, create `channel-newapi-gpt` only if channel-level pricing, visible supported models, model restrictions, or model mapping are needed now.
   ...
   - Keep `restrict_models` enabled only if the intended GPT model list is complete.
...
4. In NewAPI, create one channel named `sub2api-gpt`.
   ...
   - `models`: only the GPT names users should see/use now.
...
- `GPT5.5` has active OpenAI accounts.
