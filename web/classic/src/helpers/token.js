...
function getStoredAddresses() {
  ...
  if (status) {
    try {
      ...
      serverAddress = status.server_address || serverAddress;
      apiAddress =
        status.api_url ||
        status.api_address ||
        status.apiUrl ||
  ...
  apiAddress = (apiAddress || serverAddress).trim().replace(/\/+$/, '');

  return { apiAddress, serverAddress };
}

export function getServerAddress() {
  return getStoredAddresses().serverAddress;
}

export function getApiAddress() {
  return getStoredAddresses().apiAddress;
}
