...
export function AuthLayout({ children }: AuthLayoutProps) {
  ...
  return (
    <div className="yucore-auth-page dark relative min-h-svh overflow-hidden bg-[#05070c] text-white">
      <YucoreEntranceLoader
        className="yucore-entrance-loader-auth"
        durationMs={5800}
      />
      <YucoreBackground
        coreMode="ambient"
        intensity="hero"
        showEarthCore={false}
        className="fixed yucore-auth-background"
      />
      <YucorePersistentCore
        active
        className="yucore-persistent-core-auth yucore-persistent-core-auth-bridge"
      />
      <Link
        to="/"
      ...
      <div className="yucore-auth-stage relative z-10 mx-auto grid min-h-svh w-full max-w-7xl items-center gap-8 px-4 py-24 sm:px-8 min-[920px]:grid-cols-[1.08fr_0.92fr] min-[920px]:py-16 lg:px-10">
        <section className="yucore-auth-visual relative hidden min-h-[min(740px,calc(100svh-8rem))] min-[920px]:flex min-[920px]:flex-col min-[920px]:justify-end">
          ...
          <div className="yucore-auth-signal-field pointer-events-none absolute inset-0">
            {authMotionNodes.map(([title, detail, className]) => (
              <div
                key={title}
                className={`yucore-auth-signal ${className} absolute rounded-2xl border border-white/10 bg-black/30 px-3 py-2.5 text-left backdrop-blur-xl`}
              >
        ...
        <div className="yucore-auth-form-column mx-auto flex w-full max-w-[480px] flex-col justify-center">
          ...
          <div className="yucore-panel yucore-auth-form-panel relative overflow-hidden rounded-3xl p-5 text-foreground sm:p-8">
