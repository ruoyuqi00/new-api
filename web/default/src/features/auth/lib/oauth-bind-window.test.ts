...
describe('OAuth bind popup lifecycle', () => {
  ...
  test('expires an OAuth popup from the moment it opens and cancels on close', () => {
    const timeoutTimer = fakeTimerRuntime()
    const closeTimer = fakeTimerRuntime()
    const popup = { closed: false }
    let timeoutCount = 0
    let closedCount = 0

    const lifecycle = manageOAuthPopupLifecycle(popup, {
      onClosed: () => {
        closedCount += 1
      },
      onTimeout: () => {
      ...
      timeout: 600_000,
      timeoutRuntime: timeoutTimer.runtime,
      intervalRuntime: closeTimer.runtime,
    })

    assert.equal(timeoutTimer.delay, 600_000)
    ...
    assert.deepEqual(timeoutTimer.cancelled, [timeoutTimer.handle])
  })

  test('normal OAuth completion stops timers without aborting the request', () => {
    const timeoutTimer = fakeTimerRuntime()
    const closeTimer = fakeTimerRuntime()
    let timeoutCount = 0
    const lifecycle = manageOAuthPopupLifecycle(
      { closed: false },
      {
        onClosed: () => undefined,
        onTimeout: () => {
    ...
    assert.equal(lifecycle.beginExchange(), true)
    assert.equal(lifecycle.beginExchange(), false)
    lifecycle.complete()
    timeoutTimer.fire()
    ...
    assert.equal(lifecycle.signal.aborted, false)
    assert.deepEqual(timeoutTimer.cancelled, [timeoutTimer.handle])
    assert.deepEqual(closeTimer.cancelled, [closeTimer.handle])
  })
})
