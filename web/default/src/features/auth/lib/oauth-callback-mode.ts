...
export function resolveOAuthCallbackMode(
  ...
  if (!context.opener || !context.storage || !state) return 'login'

  try {
    if (context.opener.closed) return 'login'
    return context.storage.getItem(
...
export function isTelegramOAuthBindCallback(
  provider: string,
  result: string | undefined
): boolean {
  return provider === 'telegram' && (result === 'success' || result === 'error')
}
