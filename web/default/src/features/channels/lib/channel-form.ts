  ...
  if (channel.settings) {
    try {
      ...
      upstreamModelUpdateIgnoredModels = Array.isArray(
        ...
        : ''
      channelPoolConcurrencyLimit = Math.max(
        0,
        Number(parsed.channel_pool_concurrency_limit) || 0
      )
      channelPoolCooldownSeconds = Math.max(
        0,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if ((formData.channel_pool_concurrency_limit ?? 0) > 0) {
    settingsObj.channel_pool_concurrency_limit =
      formData.channel_pool_concurrency_limit
  } else {
    delete settingsObj.channel_pool_concurrency_limit
  }
  if ((formData.channel_pool_cooldown_seconds ?? 0) > 0) {
    settingsObj.channel_pool_cooldown_seconds =
      formData.channel_pool_cooldown_seconds
  } else {
