...
export interface YucoreMediaHealth {
  adapter: 'mock' | 'openai-compatible' | 'uag-proxy' | string
  configured: boolean
...
function getStoredCanvasIdentityToken() {
  if (typeof window === 'undefined') return ''
  return window.localStorage.getItem(fallbackCanvasIdentityTokenKey)?.trim() ?? ''
}

function getStoredCanvasIdentitySession() {
  if (typeof window === 'undefined') return ''
  return window.localStorage.getItem(fallbackCanvasIdentitySessionKey)?.trim() ?? ''
}
...
export function storeYucoreCanvasIdentity(identity: YucoreCanvasIdentity) {
  ...
  window.localStorage.setItem('yucore:canvas-identity', JSON.stringify(identity))
}
  ...
  return {
    ...
    headers: {
      ...
      ...(token
        ...
        : {}),
      ...(canvasIdentity
        ? { 'X-YuCore-Canvas-Identity': canvasIdentity }
        : {}),
...
export async function listYucoreMediaTasks(params: {
  session_id?: string
  kind?: string
  status?: string
  page_size?: number
...
export async function listYucoreMediaGallery(params: {
  kind?: string
  page_size?: number
} = {}) {
  const res = await api.get<ApiEnvelope<PageResponse<YucoreMediaTask>>>(
...
export async function listYucoreCanvases() {
  const res = await api.get<ApiEnvelope<PageResponse<YucoreCanvasRecord>>>(
    '/api/yucore/canvas',
    { params: { page_size: 20 }, disableDuplicate: true, skipErrorHandler: true }
  )
  return unwrap(res.data)
}
...
export async function listYucoreCanvasVersions(
  ...
  const res = await api.get<ApiEnvelope<PageResponse<YucoreCanvasVersionRecord>>>(
