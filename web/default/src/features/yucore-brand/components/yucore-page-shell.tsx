...
export function YucorePageShell(props: YucorePageShellProps) {
  return (
    <div
      className={cn(
        "dark relative min-h-svh overflow-hidden text-white",
        props.showBackground === false ? "bg-transparent" : "bg-[#05070c]",
        props.className,
      )}
    >
      ...
      {props.showPersistentCore && (
        <YucorePersistentCore
          active
          className={cn("yucore-persistent-core-console", props.persistentCoreClassName)}
        />
      )}
      <div
        className={cn(
