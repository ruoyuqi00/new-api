...
const BOOT_SHARD_COUNT = 380;
const BOOT_PARTICLE_COUNT = 1440;
const BOOT_SPHERE_POINT_COUNT = 380;
const DEFAULT_BOOT_DURATION_MS = 7600;
const BOOT_SPHERE_CENTER_Y_RATIO = 0.285;
...
function createBootShards() {
  return Array.from({ length: BOOT_SHARD_COUNT }, (_, index): BootShard => {
    ...
    return {
      ...
      spin: index % 2 === 0 ? 1 : -1,
      tone: index % 11 === 0 ? 2 : index % 5 === 0 ? 1 : 0,
    };
  });
}
  ...
  return Array.from(
    ...
    (_, index): SpherePoint => {
      ...
      return {
        ...
        tone: index % 13 === 0 ? 2 : index % 5 === 0 ? 1 : 0,
...
function createBootParticles() {
  return Array.from({ length: BOOT_PARTICLE_COUNT }, (_, index): BootParticle => {
    ...
    return {
      ...
      sideField,
      spin: index % 2 === 0 ? 1 : -1,
      tone: index % 17 === 0 ? 2 : index % 7 === 0 ? 1 : 0,
    };
  });
}
...
function drawCrt(
  ...
  const staticAlpha = 0.018 + smoothstep(0.14, 0.3, progress) * 0.036;
  for (let y = Math.floor((time * 22) % 12); y < height; y += 12) {
    ctx.globalAlpha = staticAlpha * (0.28 + Math.sin(y * 0.018 + time * 4) * 0.18);
  ...
  const tear = (time * 140 + Math.sin(time * 2.4) * 80) % height;
  const tearGradient = ctx.createLinearGradient(0, tear - 28, 0, tear + 28);
  tearGradient.addColorStop(0, "rgba(103, 232, 249, 0)");
  tearGradient.addColorStop(0.5, `rgba(103, 232, 249, ${0.08 + staticAlpha})`);
  tearGradient.addColorStop(1, "rgba(103, 232, 249, 0)");
  ctx.globalAlpha = 1;
  ctx.fillStyle = tearGradient;
  ctx.fillRect(0, tear - 34, width, 68);
...
function drawBrandParticleField(
  ...
  const visible =
    smoothstep(0.1, 0.24, progress) *
    (1 - smoothstep(0.98, 1.08, progress) * 0.28);
  ...
  const maxSide = Math.max(width, height);
  const corePull = smoothstep(0.54, 0.9, progress);
  const scatter = smoothstep(0.18, 0.5, progress) * (1 - smoothstep(0.8, 0.96, progress));
  const densityStride = progress < 0.22 ? 5 : progress < 0.38 ? 3 : progress < 0.58 ? 2 : 1;
  const lateOrder = smoothstep(0.68, 0.94, progress);

  ctx.save();
  ctx.globalCompositeOperation = "screen";

  for (let particleIndex = 0; particleIndex < particles.length; particleIndex += densityStride) {
    ...
    const baseRadius =
      maxSide *
      (farField
        ? 0.32 + lane * 0.52 + seed * 0.16
        : lowerGrid
          ? 0.36 + seed * 0.48
        : 0.22 + lane * 0.45 + Math.sin(seed * 12 + time) * 0.016);
    ...
    const radius = burstRadius * (1 - shellPull) + sphereParticleRadius * shellPull;
    const ellipse = 0.54 + lane * 0.16;
    const orbitalNoise = Math.sin(time * 2.4 + particle.noisePhase) * maxSide * (0.006 - lateOrder * 0.002);
    const radialX =
      centerX +
    ...
    const fieldX = farField
      ? width * (0.5 + fieldSide * (0.34 + seed * 0.18))
      : lowerGrid
        ? width * (-0.08 + particle.fieldSeedX * 1.16)
      : radialX;
    const fieldY = farField
      ? height * (0.14 + particle.fieldSeedY * 0.76)
      : lowerGrid
        ? height * (0.64 + particle.fieldSeedY * 0.28)
      : radialY;
    const fieldBlend =
      (farField
        ? 0.7 + (1 - corePull) * 0.18
        : lowerGrid
          ? 0.74 + (1 - corePull) * 0.14
          : sideField
            ? 0.34 + (1 - corePull) * 0.18
            : 0.1) *
      (1 - corePull * (farField ? 0.58 : lowerGrid ? 0.34 : 0.72));
    ...
    const globeRelief = 1 - lateOrder * (1 - smoothstep(globeRadius * 0.76, globeRadius * 1.34, distanceToCore));
    const orderedLift = lateOrder * (farField ? 0.26 : lowerGrid ? 0.2 : sideField ? 0.08 : 0);
    const alpha =
      visible *
      (0.062 + depth * (farField ? 0.27 : lowerGrid ? 0.24 : 0.16) + shellPull * 0.02 + orderedLift) *
      (coreAlphaFloor + coreClear * (1 - coreAlphaFloor)) *
      (0.36 + globeRelief * 0.64);
    const size = 0.34 + depth * (farField ? 0.92 : lowerGrid ? 0.72 : 0.92) + shellPull * 0.18;
    ...
    if (index % 31 === 0 || (farField && index % 23 === 0)) {
      ...
      const tailY = farField
        ? y - Math.sin(angle + fieldSide * 0.8) * maxSide * (0.035 + seed * 0.04)
    ...
    ctx.fillStyle = `rgba(255, 255, 255, ${Math.min(0.8, alpha * 0.58)})`;
    ctx.fillRect(
      x + size * 0.24,
      y + size * 0.24,
      Math.max(0.72, size * 0.28),
      Math.max(0.72, size * 0.28),
    );
  }
  ...
  coreGlow.addColorStop(0, `rgba(255, 255, 255, ${visible * (0.045 + corePull * 0.1)})`);
  coreGlow.addColorStop(0.32, `rgba(103, 232, 249, ${visible * (0.04 + corePull * 0.08)})`);
...
function drawSequencedPowerLanes(
  ...
  const enter = smoothstep(0.48, 0.68, progress);
  const hold = 1 - smoothstep(1.02, 1.12, progress);
  const alpha = enter * hold;

  if (alpha < 0.01) return;

  const { centerX, centerY, radius } = getBootSphereGeometry(width, height, smoothstep(0.44, 0.64, progress));
  ...
  for (let lane = 0; lane < 8; lane += 1) {
    ...
    const rotation = -0.5 + lane * 0.2 + time * direction * (0.045 + lane * 0.006);
    const laneAlpha = alpha * (0.12 + lane * 0.022);

    ctx.save();
    ctx.translate(centerX, centerY);
    ctx.rotate(rotation);
    ctx.setLineDash(lane % 2 === 0 ? [laneRadius * 0.16, laneRadius * 0.09] : [laneRadius * 0.07, laneRadius * 0.13]);
    ...
    ctx.beginPath();
    ctx.ellipse(0, 0, laneRadius * (1.22 + lane * 0.015), laneRadius * laneRatio, 0, 0, Math.PI * 2);
    ctx.stroke();
    ctx.setLineDash([]);
  ...
  for (let lane = 0; lane < 7; lane += 1) {
    ...
    ctx.beginPath();
    ctx.moveTo(startX, y);
    ctx.bezierCurveTo(width * 0.28, y - minSide * 0.06, width * 0.62, y + slope + minSide * 0.05, endX, y + slope);
    ctx.stroke();

    for (let packet = 0; packet < 3; packet += 1) {
      const t = ((time * (0.055 + lane * 0.004) + packet / 3 + lane * 0.07) % 1 + 1) % 1;
...
function drawWireFrame(
  ...
  const enter = smoothstep(0.12, 0.25, progress);
  const exit = 1 - smoothstep(0.34, 0.5, progress);
  ...
  const drawFrame = () => {
    for (let i = 0; i < 4; i += 1) {
      ...
      ctx.strokeRect(-size / 2 - grow, -size / 2 - grow, size + grow * 2, size + grow * 2);
    }
    ctx.beginPath();
...
function drawShardTunnel(
  ...
  const enter = smoothstep(0.22, 0.42, progress);
  const exit = 1 - smoothstep(0.78, 0.94, progress);
  ...
  const maxSide = Math.max(width, height);
  const minSide = Math.min(width, height);
  const fly = easeOutCubic(smoothstep(0.22, 0.68, progress));
  ...
  for (let index = 0; index < shards.length; index += 1) {
    ...
    const vibration = Math.sin(time * 3.1 + shard.jitter) * maxSide * 0.012 * burst;
    const x = centerX + Math.cos(angle) * radius + Math.cos(angle * 2.6) * vibration;
    const y =
      centerY +
  ...
  for (let lane = 0; lane < 6; lane += 1) {
    ...
    const laneRotation = -0.5 + lane * 0.24 + time * direction * (0.18 + lane * 0.025);
    const laneAlpha = vortexAlpha * (0.42 + lane * 0.08);

    ctx.save();
    ctx.translate(centerX, centerY);
    ctx.rotate(laneRotation);
    ctx.setLineDash(lane % 2 === 0 ? [laneRadius * 0.18, laneRadius * 0.09] : [laneRadius * 0.08, laneRadius * 0.12]);
    ...
    ctx.beginPath();
    ctx.ellipse(0, 0, laneRadius * 1.34, laneRadius * laneRatio, 0, 0, Math.PI * 2);
    ctx.stroke();
    ctx.setLineDash([]);

    for (let packet = 0; packet < 3; packet += 1) {
      const packetAngle = time * direction * (0.66 + lane * 0.04) + packet * 2.09 + lane;
...
function drawSphere(
  ...
  const build = smoothstep(0.44, 0.64, progress);
  const fade = 1 - smoothstep(0.985, 1.08, progress) * 0.18;
  const alpha = build * fade;

  if (alpha < 0.01) return;

  const { centerX, centerY, radius } = getBootSphereGeometry(width, height, build);
  ...
  ctx.fillRect(centerX - radius * 1.3, centerY - radius * 1.3, radius * 2.6, radius * 2.6);

  const ringAlpha = alpha * 0.32;
  ctx.lineWidth = 1;
  ctx.strokeStyle = `rgba(103, 232, 249, ${ringAlpha})`;
  ctx.beginPath();
  ctx.ellipse(centerX, centerY, radius * 1.14, radius * 0.48, time * 0.22, 0, Math.PI * 2);
  ctx.stroke();
  ctx.strokeStyle = `rgba(250, 204, 21, ${ringAlpha * 0.7})`;
  ctx.beginPath();
  ctx.ellipse(centerX, centerY, radius * 0.94, radius * 0.58, -time * 0.18, 0, Math.PI * 2);
  ctx.stroke();
  ...
  for (let lane = 0; lane < 5; lane += 1) {
    ...
    ctx.translate(centerX, centerY);
    ctx.rotate(rotation);
    ctx.setLineDash(lane % 2 === 0 ? [laneRadius * 0.18, laneRadius * 0.08] : [laneRadius * 0.08, laneRadius * 0.12]);
    ...
    ctx.beginPath();
    ctx.ellipse(0, 0, laneRadius * 1.24, laneRadius * laneRatio, 0, 0, Math.PI * 2);
    ctx.stroke();
    ctx.setLineDash([]);
    for (let packet = 0; packet < 3; packet += 1) {
      const packetAngle = time * direction * (0.64 + lane * 0.06) + packet * 2.09 + lane * 0.5;
  ...
  for (const point of spherePoints) {
    ...
    const px = centerX + rotatedX * radius * surfaceNoise * (0.92 + depth * 0.18);
    ...
    ctx.fillStyle = toneColor(point.tone, dotAlpha);
    ctx.beginPath();
    ctx.arc(px, py, dotSize * 0.42, 0, Math.PI * 2);
    ctx.fill();
  }

  const core = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, radius * 1.1);
  ...
  ctx.fillStyle = core;
  ctx.fillRect(centerX - radius * 1.18, centerY - radius * 1.18, radius * 2.36, radius * 2.36);
  ctx.restore();
}
...
function drawBootCore(
  ...
  const intro = (1 - smoothstep(0.16, 0.28, progress)) * (1 - smoothstep(0.84, 0.98, progress));
  ...
  ctx.lineWidth = 1.3;
  ctx.beginPath();
  ctx.arc(centerX, centerY, 34 + pulse * 8, time * 1.6, time * 1.6 + Math.PI * 1.38);
  ctx.stroke();
  ...
  const core = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, 72);
...
function drawBootFrame(
  ...
  if (!areFiniteNumbers(width, height, progress, time) || width <= 0 || height <= 0) {
    return;
  }

  drawBackdrop(ctx, width, height, progress, time);
  drawBrandParticleField(ctx, width, height, progress, time, particles);
  drawSequencedPowerLanes(ctx, width, height, progress, time);
  drawWireFrame(ctx, width, height, progress, time);
  drawShardTunnel(ctx, width, height, progress, time, shards);
  drawSphere(ctx, width, height, progress, time, spherePoints);
  drawBootCore(ctx, width, height, progress, time);
  drawCrt(ctx, width, height, progress, time);
}

export function YucoreBootCanvas(props: YucoreBootCanvasProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const particles = useMemo(createBootParticles, []);
  const shards = useMemo(createBootShards, []);
  const spherePoints = useMemo(createSpherePoints, []);
  const durationMs =
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      if (!reduceMotion && now - lastRenderTime < 1000 / 62) {
  ...
  }, [durationMs, particles, shards, spherePoints]);

  return (
    <canvas
      ref={canvasRef}
      className={cn("yucore-boot-canvas absolute inset-0 h-full w-full", props.className)}
      aria-hidden="true"
    />
  );
}
