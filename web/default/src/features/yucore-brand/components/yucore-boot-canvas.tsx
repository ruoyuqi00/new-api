...
function getBootGlowShadowColor(tone: number) {
  if (tone === 2) return 'rgba(250, 204, 21, 0.42)'
  if (tone === 1) return 'rgba(251, 113, 133, 0.36)'
...
function drawCrt(
  ...
  for (let y = Math.floor((time * 22) % 12); y < height; y += 12) {
    ctx.globalAlpha =
      staticAlpha * (0.28 + Math.sin(y * 0.018 + time * 4) * 0.18)
...
function drawBrandParticleField(
  ...
  const scatter =
    smoothstep(0.18, 0.5, progress) * (1 - smoothstep(0.8, 0.96, progress))
  const densityStride = getBootDensityStride(progress)
  ...
  for (
    ...
    if (particle.glow) {
      ...
      ctx.shadowColor = getBootGlowShadowColor(tone)
    }
...
function drawBootCore(
  ...
  const intro =
    (1 - smoothstep(0.16, 0.28, progress)) *
