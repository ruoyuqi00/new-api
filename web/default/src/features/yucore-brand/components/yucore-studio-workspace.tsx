...
function modelForKind(models: YucoreMediaModel[], kind: 'image' | 'video') {
  return models.find((model) => model.kind === kind)?.id ?? (kind === 'image' ? 'gpt-image-2' : 'veo-3.1-generate-preview')
}

function getModelById(models: YucoreMediaModel[], modelId: string, kind: 'image' | 'video') {
  return models.find((model) => model.id === modelId) ?? models.find((model) => model.kind === kind)
}
...
function getFormats(model?: YucoreMediaModel) {
  return model?.output_formats?.length ? model.output_formats : model?.formats?.length ? model.formats : formats
}

function inferMediaHealthFromModels(modelRows: YucoreMediaModel[]): YucoreMediaHealth | null {
  ...
  return {
    ...
    status: 'configured',
    message: 'UAG model directory is available, but real upstream provider verification is still pending.',
    ...
    upstream_verification_message: 'UAG model directory is available, but real upstream provider verification is still pending.',
...
function fallbackMediaHealth(modelRows: YucoreMediaModel[]): YucoreMediaHealth {
  return inferMediaHealthFromModels(modelRows) ?? {
    adapter: 'mock',
    configured: true,
    base_url_configured: false,
    api_key_configured: false,
    status: 'development',
    message: 'Local development media is using mock assets. Configure UAG/OpenAI-compatible upstream to switch to real assets.',
    supports_image: true,
    supports_video: true,
    ...
    upstream_verification_message: 'Local mock media does not verify an upstream provider.',
    real_workflow_ready: false,
    verification_blockers: [
...
function normalizeCanvasBackgroundMode(value: unknown): CanvasBackgroundMode {
  return value === 'lines' || value === 'dots' || value === 'blank' || value === 'net'
    ? value
    : 'net'
}
...
function cloneCanvasNodes(items: StudioCanvasNode[]) {
  return JSON.parse(JSON.stringify(items)) as StudioCanvasNode[]
}

function cloneCanvasEdges(items: Edge[]) {
  return JSON.parse(JSON.stringify(items)) as Edge[]
}
...
function importedReferenceNodeToFlowNode(row: Record<string, unknown>, index: number): StudioCanvasNode {
  ...
  const kind: CanvasNodeData['kind'] =
    rawType === 'image' || rawType === 'video' || rawType === 'prompt' ? rawType : 'text'
  const prompt = String(
    metadata.prompt || metadata.composerContent || metadata.content || row.prompt || ''
  )
  return {
    id,
    type: 'media',
...
function normalizeImportedCanvasPayload(value: unknown) {
  ...
  const snapshot = isPlainRecord(payload.snapshot) ? payload.snapshot : undefined
  const nodes = normalizeImportedCanvasNodes(snapshot?.nodes ?? payload.nodes)
  const edges = normalizeImportedCanvasEdges(snapshot?.edges ?? payload.edges ?? payload.connections)
  ...
  return {
    ...
    backgroundMode: normalizeCanvasBackgroundMode(
      snapshot?.backgroundMode ?? payload.backgroundMode ?? payload.background_mode
    ),
  }
}
...
function buildAgentRunSummary(task: YucoreMediaTask) {
  ...
  if (task.status === 'canceled') return `Media task ${task.task_id} was canceled.`
  return `Media task ${task.task_id} failed${task.error ? `: ${compactTaskError(task.error)}` : '.'}`
}

function buildAgentRunActions(run: YucoreCanvasAgentRun | undefined, task: YucoreMediaTask) {
  ...
  const actions = (run?.actions?.length ? run.actions : []).map((action) => {
    if (action.tool === 'canvas_apply_result' && action.task_id === task.task_id) {
      hasApplyResult = true
    }
    if (action.tool === 'canvas_run_generation' || (!action.tool && action.task_id === task.task_id)) {
  ...
  if (task.status === 'completed' && resultAssets.length > 0 && !hasApplyResult) {
    actions.push({
...
function AssetPreview({ asset, title }: { asset: YucoreMediaAsset; title: string }) {
  if (asset.kind === 'video') {
    return (
      <video
        src={asset.url}
    ...
    <div className='yucore-media-canvas-node'>
      ...
      <div className='flex items-start justify-between gap-3'>
        <div className='min-w-0'>
          <div className='truncate text-[13px] font-semibold text-white'>{getCanvasNodeTitle(data)}</div>
          {data.sublabel && <div className='mt-1 truncate text-[11px] text-white/42'>{data.sublabel}</div>}
        </div>
        <span
          className={cn(
            ...
            kind === 'image'
              ...
              : kind === 'video'
                ? 'border-violet-200/20 text-violet-100'
      ...
      {data.prompt && <p className='mt-2 line-clamp-3 text-[11px] leading-5 text-white/48'>{data.prompt}</p>}
      {data.status && <div className='mt-2 text-[10px] text-white/34'>{data.status}</div>}
    ...
    return (
      <div className='grid min-h-[20rem] place-items-center rounded-2xl border border-dashed border-white/12 bg-white/[0.025] text-center'>
        <div>
          ...
          <div className='mt-1 text-xs text-white/42'>从生图工作台提交后会自动沉淀到这里。</div>
        </div>
      </div>
    )
  }
        ...
        >
          ...
          <div className='space-y-2 p-3'>
            <div className='flex items-center justify-between gap-2'>
              <span className='truncate text-xs font-semibold text-white'>{asset.label}</span>
              <div className='flex shrink-0 items-center gap-1'>
                <ProviderBadge task={task} asset={asset} />
                <TaskStatusBadge task={task} />
              </div>
            </div>
            <p className='line-clamp-2 text-xs leading-5 text-white/45'>{task.prompt}</p>
            <div className='flex items-center gap-2 pt-1'>
              ...
              <Button
                ...
                variant='outline'
                className='h-7 rounded-lg border-white/10 bg-white/[0.035] px-2 text-[11px] text-white hover:bg-white/10'
                render={<a href={asset.url} download={getAssetDownloadName(asset)} />}
              >
...
export function YucoreStudioWorkspace({ initialView = 'home' }: { initialView?: StudioView }) {
  ...
  const [templates, setTemplates] = useState<YucorePromptTemplate[]>(defaultTemplates)
  ...
  const [activeCanvas, setActiveCanvas] = useState<YucoreCanvasRecord | null>(null)
  const [canvasVersions, setCanvasVersions] = useState<YucoreCanvasVersionRecord[]>([])
  const [canvasIdentity, setCanvasIdentity] = useState<YucoreCanvasIdentity | null>(null)
  ...
  const [negativePrompt, setNegativePrompt] = useState('塑料皮肤，畸形手指，多余肢体，过度 HDR，商业棚拍感')
  ...
  const [isAgentPanelOpen, setIsAgentPanelOpen] = useState(initialView === 'canvas')
  ...
  const [selectedCanvasTool, setSelectedCanvasTool] = useState<CanvasToolId>('move')
  const [selectedNodeId, setSelectedNodeId] = useState<string | null>(null)
  const [canvasViewport, setCanvasViewport] = useState<Viewport | undefined>()
  const [canvasMiniMapOpen, setCanvasMiniMapOpen] = useState(true)
  const [canvasBackgroundMode, setCanvasBackgroundMode] = useState<CanvasBackgroundMode>('net')
  const [canvasPast, setCanvasPast] = useState<CanvasHistoryEntry[]>([])
  const [canvasFuture, setCanvasFuture] = useState<CanvasHistoryEntry[]>([])
  const [reactFlowInstance, setReactFlowInstance] = useState<ReactFlowInstance<StudioCanvasNode, Edge> | null>(null)
  ...
  const activeModel = view === 'video' || kind === 'video' ? videoModel : imageModel
  const activeModelId = activeModel?.id ?? (view === 'video' || kind === 'video' ? videoModelId : imageModelId)
  const availableModels = view === 'video' || kind === 'video' ? videoModels : imageModels
  const activeModes = activeModel?.modes?.length
    ? activeModel.modes
    : view === 'video' || kind === 'video'
      ? defaultVideoModes
      : defaultImageModes
  ...
  const activeQualities = activeModel?.qualities?.length ? activeModel.qualities : defaultQualities
  const activeStylePresets = activeModel?.style_presets?.length ? activeModel.style_presets : defaultStyles
  const activeStreamModes = activeModel?.stream_modes?.length ? activeModel.stream_modes : defaultStreamModes
  const activeBackgrounds = activeModel?.backgrounds?.length ? activeModel.backgrounds : defaultBackgrounds
  const activeModerations = activeModel?.moderations?.length ? activeModel.moderations : defaultModerations
  const activeDurations = activeModel?.durations?.length ? activeModel.durations : defaultDurations
  const activeCounts = activeModel?.counts?.length ? activeModel.counts : defaultCounts
  const maxReferenceImages = activeModel?.input_limits?.max_reference_images ?? 4
  const isReferenceUploading = referenceFiles.some((file) => file.isUploading)
  const referenceUploadErrors = referenceFiles.filter((file) => file.uploadError)
  const isSubmitDisabled =
    isSubmitting || isReferenceUploading || referenceUploadErrors.length > 0 || !prompt.trim()
  const visibleTasks = useMemo(
    () => tasks.filter((task) => (view === 'video' ? task.kind === 'video' : task.kind === 'image')),
    [tasks, view]
  )
  const completedTasks = useMemo(() => {
    const seen = new Set<string>()
    return [...galleryTasks, ...tasks.filter((task) => task.status === 'completed')].filter((task) => {
  ...
  const selectedNode = useMemo(
    () => (selectedNodeId ? (nodes.find((node) => node.id === selectedNodeId) as StudioCanvasNode | undefined) : undefined),
    [nodes, selectedNodeId]
  )
  const initialCanvasViewport = useMemo(
    () => normalizeCanvasViewport(activeCanvas?.viewport),
    [activeCanvas?.id]
  )
  const effectiveCanvasViewport = useMemo(
    () => canvasViewport ?? initialCanvasViewport ?? { x: 0, y: 0, zoom: 1 },
    [canvasViewport, initialCanvasViewport]
  )
  const canvasSyncLabel = isSavingCanvas
    ? '正在同步画布'
    : isCanvasDirty
      ? activeCanvas
        ? '未保存更改，稍后自动同步'
  ...
  const canvasBackgroundModes: Array<{ id: CanvasBackgroundMode; label: string }> = [
    { id: 'net', label: '能量网' },
    { id: 'lines', label: '线网' },
    { id: 'dots', label: '点阵' },
    { id: 'blank', label: '空白' },
  ...
  const refreshTasks = useCallback(async () => {
    const payload = await listYucoreMediaTasks({ session_id: sessionId, page_size: 40 })
  ...
  const refreshCanvases = useCallback(async () => {
    ...
    setActiveCanvas((current) =>
      current && payload.items.some((item) => item.id === current.id)
        ? current
        : payload.items[0] ?? null
    )
  }, [])

  const refreshAgentRuns = useCallback(async (canvasId = activeCanvas?.id) => {
    if (!canvasId) {
      setAgentRuns([])
      return []
    }
    const payload = await listYucoreCanvasAgentRuns(canvasId, { page_size: 20 })
    setAgentRuns(payload.items)
    return payload.items
  }, [activeCanvas?.id])

  const refreshCanvasVersions = useCallback(async (canvasId = activeCanvas?.id) => {
    if (!canvasId) {
      setCanvasVersions([])
      return []
    }
    ...
    try {
      const payload = await listYucoreCanvasVersions(canvasId, { page_size: 12 })
  ...
  const openCanvasRecord = useCallback(
    async (canvas: YucoreCanvasRecord) => {
      ...
      try {
        ...
        setCanvases((items) => items.map((item) => (item.id === latest.id ? latest : item)))
    ...
    [refreshAgentRuns, refreshCanvasVersions]
  )
  ...
  async function handleRenameCanvas() {
    ...
    const nextTitle = window.prompt('输入新的画布名称', activeCanvas.title)?.trim()
    ...
    try {
      ...
      setCanvases((items) => items.map((item) => (item.id === renamed.id ? renamed : item)))
  ...
  async function handleDeleteCanvas() {
    ...
    const confirmed = window.confirm(`删除画布「${activeCanvas.title}」？这个操作不会删除已生成素材。`)
    if (!confirmed) return
    try {
      await deleteYucoreCanvas(activeCanvas.id)
      const nextCanvases = canvases.filter((item) => item.id !== activeCanvas.id)
      setCanvases(nextCanvases)
      setActiveCanvas(nextCanvases[0] ?? null)
      setCanvasVersions([])
      setAgentRuns([])
      toast.success('画布已删除')
    } catch (error) {
  ...
  async function handleRestoreCanvasVersion(version: YucoreCanvasVersionRecord) {
    if (!activeCanvas) return
    const confirmed = window.confirm(`恢复到 v${version.revision}？当前画布会保存一个新的服务端版本。`)
    if (!confirmed) return
    try {
      ...
      setCanvases((items) => items.map((item) => (item.id === restored.id ? restored : item)))
  ...
  async function handleCanvasImportChange(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0]
    event.target.value = ''
    if (!file) return
    try {
      const imported = normalizeImportedCanvasPayload(JSON.parse(await file.text()))
      if (!imported) {
      ...
      setView('canvas')
      markCanvasDirty()
      const synced = await persistCanvasSnapshot(imported.nodes, imported.edges, '导入画布', {
        autosave: false,
  ...
  useEffect(() => {
    ...
    async function load() {
      const [modelsResult, templatesResult, billingResult, healthResult, identityResult] = await Promise.allSettled([
        listYucoreMediaModels(),
        listYucorePromptTemplates(),
        getYucoreMediaBilling(),
        getYucoreMediaHealth(),
      ...
      if (modelsResult.status === 'fulfilled') {
        ...
        toast.error(modelsResult.reason instanceof Error ? modelsResult.reason.message : '加载模型列表失败')
      }

      if (templatesResult.status === 'fulfilled') {
        setTemplates(templatesResult.value.length > 0 ? templatesResult.value : defaultTemplates)
      }
      ...
      if (healthResult.status === 'fulfilled') {
        ...
        setMediaHealth({
          ...
          api_key_configured: false,
          status: 'development',
          message: '本地开发环境使用模拟素材；配置 UAG/OpenAI-compatible 后会切换为真实上游。',
          supports_image: true,
          supports_video: true,
          require_real_assets: false,
          mock_fallback: true,
      ...
      const [tasksResult, galleryResult, canvasesResult] = await Promise.allSettled([
        refreshTasks(),
        refreshGallery(),
        refreshCanvases(),
      ])
  ...
  useEffect(() => {
    ...
    setAspectRatio((current) => keepOption(current, activeAspectRatios, activeAspectRatios[0]))
    ...
    setQuality((current) => keepOption(current, activeQualities, activeQualities[0]))
    setStylePreset((current) => keepOption(current, activeStylePresets, activeStylePresets[0]))
    setBackground((current) => keepOption(current, activeBackgrounds, activeBackgrounds[0]))
    setModeration((current) => keepOption(current, activeModerations, activeModerations[0]))
    setStreamMode((current) => keepOption(current, activeStreamModes, activeStreamModes[0]))
    setDuration((current) => keepOption(current, activeDurations, activeDurations[0]))
  ...
  useEffect(() => {
    if (!activeCanvas?.snapshot) return
    const snapshotNodes = Array.isArray(activeCanvas.snapshot.nodes)
      ? (activeCanvas.snapshot.nodes as StudioCanvasNode[])
      : []
    const snapshotEdges = Array.isArray(activeCanvas.snapshot.edges)
      ? (activeCanvas.snapshot.edges as Edge[])
      : []
    if (snapshotNodes.length > 0) {
      setNodes(snapshotNodes)
    } else {
      setNodes(baseCanvasNodes)
    }
    setEdges(snapshotEdges.length > 0 ? snapshotEdges : baseCanvasEdges)
    setCanvasViewport(normalizeCanvasViewport(activeCanvas.viewport))
    setCanvasBackgroundMode(normalizeCanvasBackgroundMode(activeCanvas.snapshot.backgroundMode))
    setCanvasPast([])
    setCanvasFuture([])
    setSelectedNodeId(null)
    setIsCanvasDirty(false)
  }, [activeCanvas?.id, setEdges, setNodes])
  ...
  }, [canvasBackgroundMode, canvasViewport, edges, nodes, restoreCanvasHistoryEntry])
  ...
  const handleCanvasZoomStep = useCallback(
    (direction: -1 | 1) => {
      handleCanvasZoomChange(effectiveCanvasViewport.zoom * (direction > 0 ? 1.18 : 1 / 1.18))
    },
  ...
  const deleteSelectedCanvasNode = useCallback(() => {
    if (!selectedNodeId) return
    const confirmed = window.confirm('删除当前选中的画布节点？')
    if (!confirmed) return
    commitCanvasHistory()
    setNodes((items) => items.filter((node) => node.id !== selectedNodeId))
    setEdges((items) =>
      items.filter((edge) => edge.source !== selectedNodeId && edge.target !== selectedNodeId)
    )
    setSelectedNodeId(null)
    markCanvasDirty()
  }, [commitCanvasHistory, markCanvasDirty, selectedNodeId, setEdges, setNodes])
  ...
  const generateFromSelectedCanvasNode = useCallback(async () => {
    if (!selectedNode) return
    const nodePrompt = String(selectedNode.data.prompt || selectedNode.data.label || '').trim()
    if (!nodePrompt) {
    ...
    if (!canvasForTask) {
      try {
        canvasForTask = await createYucoreCanvas({
        ...
        setCanvases((items) => [canvasForTask!, ...items])
        setActiveCanvas(canvasForTask)
        setIsCanvasDirty(false)
      } catch (error) {
    ...
    if (taskKind === 'video') {
      setKind('video')
      setMode((current) => (current.includes('video') ? current : 'text-to-video'))
    } else {
      setKind('image')
      setMode((current) => (current.includes('image') ? current : 'text-to-image'))
    }
    ...
    try {
      const syncedCanvas = await updateYucoreCanvas(canvasForTask.id, {
        ...
        snapshot: { nodes: nextNodes, edges, backgroundMode: canvasBackgroundMode },
        viewport: canvasViewport ?? {},
        autosave: true,
      })
      ...
      toast.error(error instanceof Error ? error.message : '画布任务状态同步失败')
    }
  }, [
    activeCanvas,
    canvasBackgroundMode,
  ...
  async function handleSubmitTask(
    ...
    options: { stayOnCanvas?: boolean; metadata?: Record<string, unknown> } = {}
  ) {
    ...
    const referenceAssets = referenceFiles
      .map((file) => file.sourceUrl || file.cachedUrl || file.url || file.dataUrl)
      .filter(Boolean)
    if (referenceFiles.length > 0 && referenceAssets.length !== referenceFiles.length) {
    ...
    try {
      ...
      const submitModes = currentModel?.modes?.length
        ? currentModel.modes
        : nextKind === 'video'
          ? defaultVideoModes
          : defaultImageModes
      ...
      const task = await createYucoreMediaTask({
        kind: nextKind,
        mode: submitMode,
        model_id: currentModel?.id ?? (nextKind === 'video' ? videoModelId : imageModelId),
        prompt: taskPrompt,
  ...
  async function handleCancelTask(task: YucoreMediaTask) {
    try {
      ...
      setTasks((items) => [next, ...items.filter((item) => item.task_id !== next.task_id)])
    ...
    async (
      ...
      try {
        const canvas = targetCanvas
          ? await updateYucoreCanvas(targetCanvas.id, {
              title: targetCanvas.title,
              description: targetCanvas.description,
              module: targetCanvas.module,
              snapshot: { nodes: nextNodes, edges: nextEdges, backgroundMode: nextBackgroundMode },
              viewport: nextViewport ?? {},
              autosave,
            })
          : options.createIfMissing
            ? await createYucoreCanvas({
                title: `YuCore Canvas ${canvases.length + 1}`,
                description: '图像、提示词、素材与生成节点的无限画布。',
                module: '无限画布',
                snapshot: { nodes: nextNodes, edges: nextEdges, backgroundMode: nextBackgroundMode },
        ...
        if (!canvas) {
          setIsCanvasDirty(true)
          if (!options.quiet) toast.info(`${reason}已更新，请保存画布以便刷新后保留`)
          return null
        }
        ...
        setIsCanvasDirty(false)
        if (!options.quiet) toast.success(autosave ? `${reason}已同步到画布` : '画布已保存')
        return canvas
      } catch (error) {
        setIsCanvasDirty(true)
        toast.error(error instanceof Error ? error.message : `${reason}保存失败`)
        return null
      } finally {
    ...
    async (task: YucoreMediaTask) => {
      ...
      try {
        ...
        if (canvasId && agentRunId) {
          ...
          const nextRun = await updateYucoreCanvasAgentRun(canvasId, agentRunId, {
            mode: run?.mode ?? metadataString(task.metadata, 'agent_mode') ?? 'site',
          ...
          if (!activeCanvas || activeCanvas.id === canvasId) {
            setAgentRuns((items) => [nextRun, ...items.filter((item) => item.run_id !== nextRun.run_id)])
            setAgentTab('history')
          }
        }
        ...
        if (canvasId && taskNodeId && activeCanvas?.id === canvasId) {
          ...
          const nextNodes = sourceNodes.map((node) => {
            if (node.id === taskNodeId) {
              const nextData: CanvasNodeData = {
                ...
                label:
                  task.status === 'completed'
                    ? firstAsset?.label || node.data.label || 'Generation result'
                    : node.data.label,
                sublabel: task.model_id,
                prompt: task.prompt,
              ...
              if (task.status === 'completed' && firstAsset) {
                nextData.assetUrl = firstAsset.url || firstAsset.cached_url || firstAsset.source_url
                nextData.thumbUrl = firstAsset.thumb_url || firstAsset.cached_url || firstAsset.source_url
              }
              ...
              return {
                ...
                style: {
                  ...
                  padding: task.status === 'completed' && firstAsset ? 0 : node.style?.padding ?? 14,
                  boxShadow: '0 22px 70px rgb(0 0 0 / 0.36)',
                },
              } as StudioCanvasNode
            }
          ...
          const changed = nextNodes.some((node, index) => node !== sourceNodes[index])
          if (changed) {
            setNodes(nextNodes)
        ...
        toast.error(error instanceof Error ? error.message : 'Agent result backflow failed')
      }
    },
    [
      activeCanvas,
  ...
  useEffect(() => {
    if (!activeTask || !isActiveMediaTask(activeTask)) return

    const timer = window.setInterval(async () => {
      try {
        const next = await getYucoreMediaTask(activeTask.task_id)
        setActiveTask(next)
        setTasks((items) => [next, ...items.filter((item) => item.task_id !== next.task_id)])
      } catch {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void persistCanvasSnapshot(nodes as StudioCanvasNode[], edges, '画布变更', {
        autosave: true,
        quiet: true,
        viewport: canvasViewport,
        backgroundMode: canvasBackgroundMode,
  ...
  }, [activeCanvas, canvasBackgroundMode, canvasViewport, edges, isCanvasDirty, isSavingCanvas, nodes, persistCanvasSnapshot])
    ...
    setNodes((items) => [
      ...items,
      {
        ...
        data: {
          kind: type,
          label,
          sublabel: type === 'text' ? '新的提示词片段' : type === 'image' ? '素材 / 生成结果' : '镜头 / 运动 / 时长',
          prompt: type === 'text' ? prompt.slice(0, 160) : '从素材库拖入内容，或把生成结果发送到画布。',
        },
        style: {
          width: 190,
  ...
  async function handleReferenceChange(event: ChangeEvent<HTMLInputElement>) {
    const files = Array.from(event.target.files ?? []).slice(0, maxReferenceImages)
    referenceFilesRef.current.forEach((item) => URL.revokeObjectURL(item.previewUrl))
    ...
    event.target.value = ''
    if (drafts.length > 0 && kind === 'image' && activeModes.includes('image-to-image')) {
      setMode('image-to-image')
    }
    if (drafts.length > 0 && kind === 'video' && activeModes.includes('image-to-video')) {
    ...
    setReferenceFiles((current) => {
      const stillSameSelection =
        ...
        current.every((item, index) => item.previewUrl === drafts[index]?.previewUrl)
  ...
  async function handleAgentSubmit() {
    ...
    if (referenceUploadErrors.length > 0) {
      toast.error('Some reference uploads failed. Please replace them before running Agent.')
      return
    }
    const referenceAssets = referenceFiles
      .map((file) => file.sourceUrl || file.cachedUrl || file.url || file.dataUrl)
    ...
    const agentMediaModes = agentMediaModel?.modes?.length ? agentMediaModel.modes : defaultImageModes
    const agentMediaMode =
      referenceFiles.length > 0 && agentMediaModes.includes('image-to-image')
        ? 'image-to-image'
        : agentMediaModes.includes('text-to-image')
          ? 'text-to-image'
          : agentMediaModes[0]
    ...
    const agentNode: StudioCanvasNode = {
      id: agentNodeId,
      type: 'media',
      position: { x: Math.round(Math.random() * 260 - 180), y: Math.round(Math.random() * 220 - 110) },
      data: {
        kind: 'prompt',
        label: 'Agent 指令',
    ...
    const canvasForRun = await persistCanvasSnapshot(plannedNodes, plannedEdges, 'Agent flow', {
      autosave: true,
      createIfMissing: true,
      quiet: true,
      viewport: canvasViewport,
    ...
    try {
      ...
      setTasks((items) => [result.task, ...items.filter((item) => item.task_id !== result.task.task_id)])
      setAgentRuns((items) => [result.run, ...items.filter((item) => item.run_id !== result.run.run_id)])
  ...
  const canvasAgentTabs: Array<{ id: CanvasAgentTab; label: string; icon: LucideIcon }> = [
  ...
  const canvasMenuItems: CanvasMenuItem[] = [
    ...
    { icon: GalleryHorizontalEnd, label: '我的画布', action: () => openCanvasView('canvas') },
    { icon: ImagePlus, label: '生图工作台', action: () => openCanvasView('image') },
    { icon: Clapperboard, label: '视频创作台', action: () => openCanvasView('video') },
    { icon: FileText, label: '提示词库', action: () => openCanvasView('prompts') },
    { icon: LibraryBig, label: '我的素材', action: () => openCanvasView('assets') },
    { icon: Plus, label: '新建画布', action: handleCreateCanvas },
    { icon: RefreshCw, label: '刷新画布', action: () => void handleRefreshCanvas() },
    { icon: FileText, label: '重命名画布', action: () => void handleRenameCanvas(), disabled: !activeCanvas },
    { icon: Save, label: '保存画布', action: () => void handleSaveCanvas() },
    { icon: Upload, label: '导入画布', action: () => canvasImportInputRef.current?.click() },
    ...
    { icon: Eraser, label: '清空画布', danger: true, action: clearCanvas, separatorBefore: true },
    { icon: Eraser, label: '删除服务端画布', danger: true, disabled: !activeCanvas, action: () => void handleDeleteCanvas() },
  ]

  const canvasToolbarItems: Array<{ id: CanvasToolId; icon: LucideIcon; label: string; action: () => void; disabled?: boolean }> = [
    { id: 'move', icon: MousePointer2, label: '移动/选择', action: () => setSelectedCanvasTool('move') },
    { id: 'save', icon: Save, label: '保存画布', action: () => void handleSaveCanvas() },
    { id: 'undo', icon: Undo2, label: '撤销', disabled: !canUndoCanvas, action: undoCanvas },
    { id: 'redo', icon: Redo2, label: '重做', disabled: !canRedoCanvas, action: redoCanvas },
    { id: 'text', icon: Type, label: '文本', action: () => addCanvasNode('text') },
    { id: 'image', icon: ImagePlus, label: '图片', action: () => addCanvasNode('image') },
    { id: 'video', icon: Clapperboard, label: '视频', action: () => addCanvasNode('video') },
    { id: 'audio', icon: Music2, label: '音频', action: () => toast.info('音频素材节点将在视频链路中接入') },
    { id: 'settings', icon: SlidersHorizontal, label: '生成配置', action: () => setView('image') },
    { id: 'upload', icon: Upload, label: '导入画布', action: () => canvasImportInputRef.current?.click() },
    { id: 'assets', icon: FolderOpen, label: '我的素材', action: () => setView('assets') },
    { id: 'organize', icon: LayoutGrid, label: '整理画布', action: organizeCanvas },
    { id: 'appearance', icon: Palette, label: '画布外观', action: () => handleCanvasBackgroundModeChange(canvasBackgroundMode === 'net' ? 'lines' : 'net') },
          ...
          <header className='z-20 flex h-14 shrink-0 items-center justify-between gap-3 border-b border-white/10 bg-black/62 px-4 backdrop-blur-2xl'>
            <div className='flex min-w-0 items-center gap-3'>
              ...
              <div className='hidden min-w-0 sm:block'>
                <div className='truncate text-sm font-semibold text-white'>YuCore Pixel</div>
                <div className='truncate text-[11px] text-white/38'>{YUCORE_STUDIO_NAME} / 用户端创作工作区</div>
              </div>
            </div>
        ...
        <div className='flex min-h-0 flex-1'>
          ...
          <main className='min-h-0 flex-1 overflow-hidden'>
                ...
                <div className='mx-auto grid min-h-[calc(100svh-8rem)] max-w-7xl content-center gap-8'>
                  <div className='max-w-4xl'>
                    ...
                    <p className='mt-6 max-w-3xl text-lg leading-8 text-white/58'>
                      在 YuCore 中生成、连接和重组图片、文字与图形，让创作从一次生成变成连续推演。
                    </p>
                  ...
                  <div className='grid gap-4 lg:grid-cols-[1.15fr_0.85fr]'>
                    <section className='rounded-2xl border border-white/10 bg-black/42 p-5 backdrop-blur-xl'>
                      <div className='mb-4 flex items-center justify-between gap-3'>
                        <div>
                          <h2 className='text-2xl font-semibold text-white'>沉淀每一次好结果</h2>
                          <p className='mt-1 text-sm text-white/48'>收藏稳定出图的提示词、参考风格和结果图片。</p>
                        </div>
                        <Button
                          variant='outline'
                      ...
                      <AssetGrid tasks={completedTasks.slice(0, 4)} compact onSendToCanvas={addAssetToCanvas} />
                    </section>
            ...
            {view === 'canvas' && (
              <section
                className={cn(
                  'yucore-canvas-motion-stage relative h-full min-h-0 overflow-hidden bg-[#10100f]/58',
                  `yucore-canvas-motion-stage-${canvasBackgroundMode}`
                )}
              >
                <div className='yucore-canvas-particle-field absolute inset-0' aria-hidden='true' />
                <ReactFlow
                  ...
                  defaultViewport={initialCanvasViewport}
                  fitView={!initialCanvasViewport}
                ...
                >
                  {canvasBackgroundMode !== 'blank' && (
                    <Background
                      variant={canvasBackgroundMode === 'dots' ? BackgroundVariant.Dots : BackgroundVariant.Lines}
                ...
                <div className='absolute inset-x-0 top-0 z-20 flex h-14 items-center justify-between border-b border-white/10 bg-[#161512]/82 px-4 backdrop-blur-xl'>
                  <div className='flex min-w-0 items-center gap-3'>
                    <button
                      type='button'
                      className={cn(
                    ...
                    <span className='hidden text-[11px] text-white/34 sm:inline'>{canvasSyncLabel}</span>
                  </div>
                  <div className='flex items-center gap-1.5'>
                    <button type='button' className='flex size-8 items-center justify-center rounded-lg text-white/58 hover:bg-white/10 hover:text-white' aria-label='配置' title='配置'>
                      <SlidersHorizontal className='size-4' />
                    </button>
                    <button type='button' className='flex size-8 items-center justify-center rounded-lg text-white/58 hover:bg-white/10 hover:text-white' aria-label='切换到浅色主题' title='切换到浅色主题'>
                      <Sun className='size-4' />
                    </button>
                    <button type='button' className='hidden size-8 items-center justify-center rounded-lg text-white/58 hover:bg-white/10 hover:text-white sm:flex' aria-label='快捷键' title='快捷键'>
                ...
                {isCanvasMenuOpen && (
                  <div className='yucore-canvas-menu absolute top-16 left-4 z-30 w-64 rounded-2xl border border-white/10 bg-[#181716]/95 p-2 shadow-[0_28px_80px_rgb(0_0_0/0.42)] backdrop-blur-2xl'>
                    {canvasMenuItems.map((item, index) => {
                      const Icon = item.icon
                      const hasDivider = item.separatorBefore || index === 6
                      return (
                        <div key={item.label}>
                          {hasDivider && <div className='my-1 h-px bg-white/10' />}
                          <button
                            type='button'
                          ...
                          >
                            ...
                            {item.shortcut && <span className='font-mono text-[11px] text-white/28'>{item.shortcut}</span>}
                          </button>
                        </div>
                      )
                    })}
                ...
                {selectedNode && (
                  <aside className='absolute top-20 left-5 z-20 flex max-h-[calc(100%-7rem)] w-[min(22rem,calc(100vw-2.5rem))] flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#171614]/92 text-white shadow-[0_24px_90px_rgb(0_0_0/0.46)] backdrop-blur-2xl'>
                    <div className='flex shrink-0 items-center justify-between gap-3 border-b border-white/10 px-4 py-3'>
                      <div className='min-w-0'>
                        <div className='truncate text-sm font-semibold'>节点检查器</div>
                        <div className='mt-1 truncate text-[11px] text-white/38'>{selectedNode.id}</div>
                      </div>
                      <button
                        type='button'
                    ...
                    <div className='min-h-0 flex-1 overflow-y-auto p-4'>
                      <label className='block text-xs font-medium text-white/52'>
                        ...
                        <input
                          ...
                          onChange={(event) => updateSelectedCanvasNode({ label: event.target.value })}
                          className='mt-2 h-10 w-full rounded-xl border border-white/10 bg-black/34 px-3 text-sm text-white outline-none transition placeholder:text-white/28 focus:border-cyan-200/45'
                        />
                      </label>
                      <label className='mt-4 block text-xs font-medium text-white/52'>
                        ...
                        <input
                          ...
                          onChange={(event) => updateSelectedCanvasNode({ sublabel: event.target.value })}
                          className='mt-2 h-10 w-full rounded-xl border border-white/10 bg-black/34 px-3 text-sm text-white outline-none transition placeholder:text-white/28 focus:border-cyan-200/45'
                        />
                      </label>
                      <div className='mt-4'>
                        <div className='mb-2 text-xs font-medium text-white/52'>节点类型</div>
                        <div className='grid grid-cols-4 gap-1 rounded-xl border border-white/10 bg-black/28 p-1'>
                          {(['text', 'prompt', 'image', 'video'] as const).map((item) => (
                            <button
                              key={item}
                              type='button'
                              onClick={() => updateSelectedCanvasNode({ kind: item })}
                              className={cn(
                      ...
                      <label className='mt-4 block text-xs font-medium text-white/52'>
                        ...
                        <textarea
                          ...
                          onChange={(event) => updateSelectedCanvasNode({ prompt: event.target.value })}
                          className='mt-2 min-h-28 w-full resize-none rounded-xl border border-white/10 bg-black/34 px-3 py-2 text-sm leading-6 text-white outline-none transition placeholder:text-white/28 focus:border-cyan-200/45'
                        />
                      </label>
                    ...
                    <div className='grid shrink-0 grid-cols-2 gap-2 border-t border-white/10 p-3'>
                      ...
                      >
                        {isSubmitting ? <Loader2 data-icon='inline-start' className='animate-spin' /> : <WandSparkles data-icon='inline-start' />}
                        从节点生成
                      </Button>
                    </div>
                  </aside>
                ...
                <div className='absolute bottom-20 left-5 z-20 flex max-w-[calc(100vw-2.5rem)] items-center gap-2 overflow-x-auto rounded-2xl border border-white/10 bg-black/64 px-3 py-2 text-white/70 backdrop-blur-2xl xl:bottom-5 xl:max-w-[calc(100vw-10rem)]'>
                  ...
                  <div className='mx-1 flex items-center gap-1 rounded-xl border border-white/10 bg-white/[0.035] p-1'>
                    {canvasBackgroundModes.map((item) => (
                      <button
                        ...
                        type='button'
                        className={cn(
                          'h-7 rounded-lg px-2 text-[11px] transition',
                          canvasBackgroundMode === item.id
                            ? 'bg-white text-black'
                            : 'text-white/52 hover:bg-white/10 hover:text-white'
                        )}
                        onClick={() => handleCanvasBackgroundModeChange(item.id)}
                      >
                        {item.label}
                      </button>
                    ))}
                  ...
                  <input
                    ...
                    onChange={(event) => handleCanvasZoomChange(Number(event.target.value) / 100)}
                  />
                  <span className='font-mono text-xs text-white/76'>{canvasZoomPercent}%</span>
                  <button
                    type='button'
                    className='flex size-8 items-center justify-center rounded-xl hover:bg-white/10'
                    aria-label='放大画布'
                ...
                <div className='yucore-studio-floating-toolbar absolute bottom-5 left-1/2 z-10 flex max-w-[calc(100vw-2rem)] -translate-x-1/2 items-center gap-1 overflow-x-auto rounded-2xl border border-white/10 bg-[#171614]/86 p-2 backdrop-blur-2xl'>
                  {canvasToolbarItems.map(({ id, icon: Icon, label, action, disabled }) => (
                    <button
                      key={id}
                      type='button'
                      disabled={disabled}
                      ...
                      className={cn(
                        ...
                        disabled && 'cursor-not-allowed opacity-32 hover:bg-transparent hover:text-white/58'
                      )}
                      aria-label={label}
                      title={label}
                    >
                ...
                {isAgentPanelOpen && (
                  <aside className='yucore-canvas-agent-panel absolute inset-y-0 right-0 z-30 flex w-[min(34rem,calc(100vw-4rem))] flex-col border-l border-white/10 bg-[#171614]/72 text-white shadow-[-18px_0_70px_rgb(0_0_0/0.36)] backdrop-blur-xl md:w-[min(34rem,calc(100vw-18rem))]'>
                    <div className='flex h-14 shrink-0 items-center justify-between border-b border-white/10 px-4'>
                      <div className='flex items-center gap-3'>
                        ...
                        <div>
                          ...
                          <div className='text-[11px] text-white/40'>画布助手</div>
                        </div>
                      </div>
                      <div className='flex items-center gap-2'>
                          ...
                          {(['site', 'local'] as const).map((item) => (
                            <button
                              ...
                              className={cn(
                                ...
                                agentMode === item ? 'bg-white text-black' : 'text-white/58 hover:bg-white/10 hover:text-white'
                              )}
                            >
                              {item === 'site' ? '网站' : '本机'}
                            </button>
                        ...
                        <label className='hidden items-center gap-2 text-xs text-white/58 sm:flex'>
                          <input
                            ...
                            onChange={(event) => setToolConfirmationEnabled(event.target.checked)}
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              ...
                              agentTab === item.id ? 'bg-white text-black' : 'text-white/58 hover:bg-white/10 hover:text-white'
                            )}
                          >
                            <Icon className='size-4' />
                            <span>{item.label}</span>
                    ...
                    <div className='min-h-0 flex-1 overflow-y-auto p-5'>
                      {agentTab === 'connect' && (
                        <div className='grid gap-3'>
                          <div className='rounded-2xl border border-white/10 bg-white/[0.035] p-4'>
                            <div className='text-sm font-semibold'>媒体引擎</div>
                            <div className='mt-2 text-sm text-white/58'>
                              {mediaEngineLabel(mediaHealth)}
                            </div>
                            {mediaHealth?.message && <p className='mt-3 text-xs leading-5 text-amber-100/70'>{mediaHealth.message}</p>}
                            {mediaHealth?.verification_blockers?.length ? (
                              <div className='mt-3 grid gap-1.5'>
                                {mediaHealth.verification_blockers.slice(0, 4).map((blocker) => (
                                  <div key={blocker} className='rounded-lg border border-amber-200/12 bg-amber-300/8 px-2.5 py-2 text-xs leading-5 text-amber-100/68'>
                                    {blocker}
                                  </div>
                                ))}
                              </div>
                          ...
                          <div className='rounded-2xl border border-white/10 bg-white/[0.035] p-4'>
                            <div className='flex items-center justify-between gap-3'>
                              <div className='text-sm font-semibold'>身份桥</div>
                              <span className={cn(
                            ...
                            <div className='mt-3 grid gap-2 text-xs text-white/46'>
                              ...
                              <div className='flex items-center justify-between gap-3'>
                                ...
                                <span className='font-mono text-white/62'>
                                  {canvasIdentity ? formatTime(canvasIdentity.expires_at) : 'none'}
                                </span>
                              </div>
                            </div>
                          </div>
                          <div className='rounded-2xl border border-white/10 bg-white/[0.035] p-4'>
                            <div className='flex items-center justify-between gap-3'>
                              <div>
                                <div className='text-sm font-semibold'>画布库</div>
                                <div className='mt-1 text-[11px] text-white/38'>服务端项目、快照与版本记录</div>
                              </div>
                            ...
                            <div className='mt-3 grid gap-2'>
                              {canvases.length === 0 ? (
                                <div className='rounded-xl border border-dashed border-white/12 p-3 text-xs text-white/42'>当前是本地草稿，保存后会出现在这里。</div>
                              ) : (
                                canvases.slice(0, 6).map((canvas) => (
                                  <button
                                    key={canvas.id}
                                    type='button'
                                    onClick={() => void openCanvasRecord(canvas)}
                                    className={cn(
                                      'rounded-xl border p-3 text-left text-sm transition',
                                      activeCanvas?.id === canvas.id ? 'border-cyan-200/25 bg-cyan-300/10' : 'border-white/10 bg-black/28 hover:bg-white/[0.06]'
                                    )}
                                  >
                                    <div className='truncate font-semibold'>{canvas.title}</div>
                                    <div className='mt-1 text-[11px] text-white/38'>
                                      {Array.isArray(canvas.snapshot?.nodes) ? canvas.snapshot.nodes.length : 0} 节点 / v{canvas.revision}
                                    </div>
                                  </button>
                                ))
                              )}
                          ...
                          <div className='rounded-2xl border border-white/10 bg-white/[0.035] p-4'>
                            <div className='flex items-center justify-between gap-3'>
                              <div>
                                ...
                                <div className='mt-1 text-[11px] text-white/38'>手动保存会生成可恢复版本</div>
                              </div>
                              {activeCanvas && (
                            ...
                            <div className='mt-3 grid gap-2'>
                              {!activeCanvas ? (
                                <div className='rounded-xl border border-dashed border-white/12 p-3 text-xs text-white/42'>保存画布后会显示版本。</div>
                              ) : isLoadingCanvasVersions ? (
                                <div className='rounded-xl border border-white/10 bg-black/28 p-3 text-xs text-white/42'>正在加载版本记录...</div>
                              ) : canvasVersions.length === 0 ? (
                                <div className='rounded-xl border border-dashed border-white/12 p-3 text-xs text-white/42'>暂无版本记录。</div>
                              ) : (
                                canvasVersions.slice(0, 6).map((version) => (
                                  <button
                                    ...
                                    onClick={() => void handleRestoreCanvasVersion(version)}
                                  >
                                    <div className='flex items-center justify-between gap-3'>
                                      <span className='font-mono text-xs text-cyan-100'>v{version.revision}</span>
                                      <span className='text-[11px] text-white/34'>{formatTime(version.created_time)}</span>
                                    </div>
                                    <div className='mt-1 text-[11px] text-white/42'>
                                      {Array.isArray(version.snapshot?.nodes) ? version.snapshot.nodes.length : 0} 节点 / {version.module || '无限画布'}
                                    </div>
                                  </button>
                                ))
                              )}
                      ...
                      {agentTab === 'chat' && (
                        <div className='grid h-full place-items-center text-center'>
                          <div>
                            <YucoreBrandMark />
                            <h2 className='mt-6 text-4xl font-semibold'>光合像素</h2>
                            <p className='mt-3 font-serif text-lg italic text-white/50'>像素绽放，灵感流动</p>
                            <p className='mx-auto mt-5 max-w-md text-sm leading-6 text-white/44'>
                              描述你想让 Agent 如何操作画布。当前会把指令作为生图任务提交，并把结果回流到素材与画布。
                            </p>
                          </div>
                        </div>
                      )}
                          ...
                          {agentRuns.slice(0, 8).map((run) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                const task = tasks.find((item) => item.task_id === run.result_task_id)
                            ...
                            >
                              <div className='mb-2 flex items-center justify-between gap-2'>
                                ...
                                <span className='font-mono text-[11px] text-white/34'>{run.run_id.slice(-8)}</span>
                              </div>
                              <div className='line-clamp-2 text-sm text-white/68'>{run.prompt}</div>
                              <div className='mt-2 text-[11px] text-white/34'>
                                {run.summary || 'Agent 工具运行'} / {formatTime(run.updated_time)}
                              </div>
                            </button>
                          ))}
                          {agentRuns.length === 0 && visibleTasks.length === 0 ? (
                            <div className='rounded-2xl border border-dashed border-white/12 p-5 text-sm text-white/44'>暂无 Agent 任务历史。</div>
                          ) : (
                            visibleTasks.slice(0, 8).map((task) => (
                              ...
                              >
                                ...
                                <div className='line-clamp-2 text-sm text-white/68'>{task.prompt}</div>
                                <div className='mt-2 text-[11px] text-white/34'>{formatTime(task.updated_time)}</div>
                              </button>
                            ))
                          )}
                        </div>
                      ...
                      {agentTab === 'logs' && (
                        <div className='rounded-2xl border border-white/10 bg-black/38 p-4 font-mono text-xs leading-6 text-white/54'>
                          <div>[canvas] nodes={nodes.length} edges={edges.length}</div>
                          <div>[sync] {canvasSyncLabel}</div>
                          <div>[mode] agent={agentMode} confirmation={toolConfirmationEnabled ? 'on' : 'off'}</div>
                          <div>
                            [media] adapter={mediaHealth?.adapter ?? 'loading'} ready=
                            {mediaHealth?.real_workflow_ready ? 'true' : 'false'} status=
                            {mediaHealth?.upstream_verification_status ?? 'loading'}
                          </div>
                          {agentRuns.slice(0, 3).map((run) => (
                            <div key={run.run_id} className='mt-3 border-t border-white/10 pt-3'>
                              <div>[agent] {run.run_id} {run.status}</div>
                              <div>[task] {run.result_task_id || 'none'}</div>
                              {run.actions.slice(-8).map((action, index) => (
                                <div key={`${run.run_id}-${index}`}>
                      ...
                      <div className='rounded-2xl border border-white/10 bg-black/38 p-3'>
                        ...
                        <div className='mt-3 flex items-center justify-between gap-3'>
                          <div className='flex items-center gap-2 text-xs text-white/42'>
                            ...
                            <span>{activeModel?.name ?? 'gpt-image-2'} · 光合像素</span>
                          </div>
                          <Button
              ...
              <section className='grid h-full min-h-0 gap-3 overflow-hidden p-3 xl:grid-cols-[18rem_23rem_minmax(0,1fr)]'>
                <aside className='hidden min-h-0 overflow-hidden rounded-2xl border border-white/10 bg-black/40 xl:block'>
                  <div className='flex items-center justify-between border-b border-white/10 p-4'>
                    <h2 className='text-sm font-semibold text-white'>生成记录</h2>
                  ...
                  <div className='flex gap-2 border-b border-white/10 p-3'>
                    <Button size='sm' className='h-8 rounded-lg bg-white text-black hover:bg-cyan-50' disabled={isSubmitDisabled} onClick={() => handleSubmitTask(view === 'video' ? 'video' : 'image')}>
                      <Plus data-icon='inline-start' />
                      新建
                    </Button>
                    <Button size='sm' variant='outline' className='h-8 rounded-lg border-white/10 bg-white/[0.035] text-white hover:bg-white/10' onClick={refreshTasks}>
                      ...
                      visibleTasks.map((task) => (
                        ...
                        >
                          <div className='mb-2 flex items-center justify-between gap-2'>
                            ...
                            <span className='text-[11px] text-white/34'>{task.progress}%</span>
                          </div>
                          <div className='line-clamp-2 text-xs leading-5 text-white/62'>{task.prompt}</div>
                          <div className='mt-2 text-[11px] text-white/32'>{formatTime(task.created_time)}</div>
                        </button>
                      ))
                    )}
                  </div>
                ...
                <section className='min-h-0 overflow-y-auto rounded-2xl border border-white/10 bg-black/42 p-4 backdrop-blur-xl'>
                  <div className='mb-4 flex items-start justify-between gap-3'>
                    <div>
                      <h1 className='text-2xl font-semibold text-white'>{view === 'video' ? '视频创作台' : '生图工作台'}</h1>
                  ...
                  <div className='mb-4 grid grid-cols-2 gap-2 rounded-xl border border-white/10 bg-white/[0.035] p-3 text-xs'>
                    <div>
                      ...
                      <div className='mt-1 font-mono text-sm text-white'>{billing?.available_points ?? '--'}</div>
                    </div>
                    <div>
                      <div className='text-white/34'>结算模式</div>
                      <div className='mt-1 text-sm text-white'>{billing?.active_mode === 'native_wallet' ? '站内钱包' : '额度同步'}</div>
                    </div>
                    <div>
                  ...
                  {mediaHealth?.verification_blockers?.length ? (
                    <div className='mb-4 rounded-xl border border-rose-200/15 bg-rose-400/[0.055] px-3 py-2 text-xs leading-5 text-rose-50/78'>
                      <div className='font-semibold text-rose-50'>真实工作流阻断项</div>
                  ...
                  <div className='grid gap-4'>
                    <div className='grid gap-2'>
                      <div className='flex items-center justify-between gap-2'>
                        <label htmlFor='yucore-prompt' className='text-sm font-semibold text-white'>提示词</label>
                        <div className='flex gap-2'>
                          <Button size='sm' variant='outline' className='h-8 rounded-lg border-white/10 bg-white/[0.035] text-white hover:bg-white/10' onClick={() => setView('prompts')}>
                            提示词库
                          </Button>
                          <Button size='sm' variant='outline' className='h-8 rounded-lg border-white/10 bg-white/[0.035] text-white hover:bg-white/10' onClick={() => setView('assets')}>
                            我的素材
                          </Button>
                        </div>
                      </div>
                    ...
                    <div className='grid gap-2'>
                      <label htmlFor='yucore-negative' className='text-sm font-semibold text-white'>负面提示词</label>
                      <textarea
                        id='yucore-negative'
                    ...
                    <div className='rounded-xl border border-dashed border-white/12 bg-white/[0.025] p-4'>
                      <div className='mb-3 flex items-center justify-between gap-2'>
                        <div>
                          <div className='text-sm font-semibold text-white'>参考图</div>
                          <div className='mt-1 text-[11px] text-white/34'>
                            最多 {maxReferenceImages} 张，当前 {referenceFiles.length} 张
                          </div>
                        </div>
                        <div className='flex gap-2'>
                          {referenceFiles.length > 0 && (
                            <Button size='sm' variant='outline' className='h-8 rounded-lg border-white/10 bg-white/[0.035] text-white hover:bg-white/10' onClick={clearReferenceFiles}>
                              清空
                            </Button>
                          )}
                          <Button size='sm' variant='outline' className='h-8 rounded-lg border-white/10 bg-white/[0.035] text-white hover:bg-white/10' disabled={isReferenceUploading} onClick={() => fileInputRef.current?.click()}>
                        ...
                        <div className='grid grid-cols-3 gap-2'>
                          {referenceFiles.map((file) => (
                            ...
                            >
                              ...
                              <div className='truncate px-2 py-1.5 text-[11px] text-white/45'>{file.name}</div>
                            </div>
                          ))}
                        </div>
                      ) : (
                    ...
                    <div className='grid gap-3'>
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>模型</div>
                        <div className='grid gap-2'>
                            ...
                            availableModels.map((model) => (
                              ...
                              >
                                <div className='flex items-center justify-between gap-2'>
                                  ...
                                  {model.badge && <span className='rounded-md border border-white/10 px-2 py-0.5 text-[10px] text-white/45'>{model.badge}</span>}
                                </div>
                                {model.description && <p className='mt-1 line-clamp-2 text-[11px] leading-5 text-white/42'>{model.description}</p>}
                              </button>
                            ))
                          )}
                        </div>
                      ...
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>模式</div>
                        <div className='flex flex-wrap gap-2'>
                          {activeModes.map((item) => (
                            <SegmentedButton key={item} active={mode === item} onClick={() => setMode(item)}>
                      ...
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>宽高比</div>
                        <div className='flex flex-wrap gap-2'>
                          {activeAspectRatios.map((item) => (
                            <SegmentedButton key={item} active={aspectRatio === item} onClick={() => setAspectRatio(item)}>
                              {item === 'auto' ? '自动' : item}
                            </SegmentedButton>
                          ))}
                        </div>
                      ...
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>{activeModel?.size_label ?? '尺寸'}</div>
                        <div className='flex flex-wrap gap-2'>
                          {activeSizes.map((item) => (
                            <SegmentedButton key={item} active={size === item} onClick={() => setSize(item)}>
                      ...
                      {view === 'image' && (
                        <div>
                          <div className='mb-2 text-sm font-semibold text-white'>格式</div>
                          <div className='flex flex-wrap gap-2'>
                            {activeFormats.map((item) => (
                              <SegmentedButton key={item} active={format === item} onClick={() => setFormat(item)}>
                      ...
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>质量</div>
                        <div className='flex flex-wrap gap-2'>
                          {activeQualities.map((item) => (
                            <SegmentedButton key={item} active={quality === item} onClick={() => setQuality(item)}>
                              {item === 'auto' ? '自动' : item}
                            </SegmentedButton>
                          ))}
                        </div>
                      ...
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>风格预设</div>
                        <div className='flex flex-wrap gap-2'>
                          {activeStylePresets.map((item) => (
                            <SegmentedButton key={item} active={stylePreset === item} onClick={() => setStylePreset(item)}>
                      ...
                      {view === 'image' && activeBackgrounds.length > 0 && (
                        <div>
                          <div className='mb-2 text-sm font-semibold text-white'>背景</div>
                          <div className='flex flex-wrap gap-2'>
                            {activeBackgrounds.map((item) => (
                              <SegmentedButton key={item} active={background === item} onClick={() => setBackground(item)}>
                      ...
                      {view === 'image' && activeModerations.length > 0 && (
                        <div>
                          <div className='mb-2 text-sm font-semibold text-white'>内容审核</div>
                          <div className='flex flex-wrap gap-2'>
                            {activeModerations.map((item) => (
                              <SegmentedButton key={item} active={moderation === item} onClick={() => setModeration(item)}>
                      ...
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>执行方式</div>
                        <div className='flex flex-wrap gap-2'>
                          {activeStreamModes.map((item) => (
                            <SegmentedButton key={item} active={streamMode === item} onClick={() => setStreamMode(item)}>
                      ...
                      {view === 'image' && streamMode === 'partial' && (
                        <div>
                          <div className='mb-2 text-sm font-semibold text-white'>预览阶段</div>
                          <div className='flex flex-wrap gap-2'>
                            {(activeModel?.partial_images?.length ? activeModel.partial_images : [0, 1, 2, 3]).map((item) => (
                              <SegmentedButton key={item} active={partialImages === item} onClick={() => setPartialImages(item)}>
                                {item === 0 ? '关闭' : `${item} 段`}
                              </SegmentedButton>
                            ))}
                          </div>
                      ...
                      {view === 'video' && (
                        <div>
                          <div className='mb-2 text-sm font-semibold text-white'>时长</div>
                          <div className='flex flex-wrap gap-2'>
                            {activeDurations.map((item) => (
                              <SegmentedButton key={item} active={duration === item} onClick={() => setDuration(item)}>
                                {item} 秒
                              </SegmentedButton>
                            ))}
                          </div>
                      ...
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>审核</div>
                        <div className='flex flex-wrap gap-2'>
                          {safetyModes.map((item) => (
                            <SegmentedButton key={item} active={safetyMode === item} onClick={() => setSafetyMode(item)}>
                              {moderationLabels[item] ?? item}
                            </SegmentedButton>
                          ))}
                        </div>
                      ...
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>安全等级</div>
                        <div className='flex flex-wrap gap-2'>
                          {Object.keys(safetyLevelLabels).map((item) => (
                            <SegmentedButton key={item} active={safetyLevel === item} onClick={() => setSafetyLevel(item)}>
                      ...
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>可见性</div>
                        <div className='flex flex-wrap gap-2'>
                          {Object.keys(visibilityLabels).map((item) => (
                            <SegmentedButton key={item} active={visibility === item} onClick={() => setVisibility(item)}>
                      ...
                      <div>
                        <div className='mb-2 text-sm font-semibold text-white'>失败重试</div>
                        <div className='flex flex-wrap gap-2'>
                          {retryCounts.map((item) => (
                            <SegmentedButton key={item} active={retryCount === item} onClick={() => setRetryCount(item)}>
                              {item}
                            </SegmentedButton>
                          ))}
                        </div>
                      ...
                      {view === 'image' && (
                        <div>
                          <div className='mb-2 text-sm font-semibold text-white'>生成张数</div>
                          <div className='flex items-center gap-2'>
                            {activeCounts.map((item) => (
                              <SegmentedButton key={item} active={count === item} onClick={() => setCount(item)}>
                                {item}
                              </SegmentedButton>
                            ))}
                          </div>
                    ...
                    <Button
                      ...
                      onClick={() => handleSubmitTask(view === 'video' ? 'video' : 'image')}
                    >
                      {isSubmitting ? <Loader2 data-icon='inline-start' className='animate-spin' /> : <Play data-icon='inline-start' />}
                      开始生成
                    </Button>
                  </div>
                </section>
                  ...
                  {!activeTask ? (
                    <div className='grid min-h-[34rem] place-items-center rounded-2xl border border-dashed border-white/12 bg-white/[0.025] text-center'>
                      <div>
                        ...
                        <div className='text-sm font-semibold text-white'>还没有生成图片</div>
                        <div className='mt-1 text-xs text-white/42'>提交任务后会在这里显示进度和结果。</div>
                      </div>
                    </div>
                  ) : isActiveMediaTask(activeTask) ? (
                    <div className='grid min-h-[34rem] place-items-center rounded-2xl border border-cyan-200/14 bg-cyan-300/[0.045] text-center'>
                      <div className='w-full max-w-sm'>
                        ...
                        <div className='text-sm font-semibold text-white'>
                          {activeTask.status === 'pending' ? '任务排队中' : '任务处理中'}
                        </div>
                        ...
                        <div className='mt-2 text-xs text-white/42'>{activeTask.task_id}</div>
                        <Button
                          variant='outline'
                    ...
                    <AssetGrid tasks={[activeTask]} onSendToCanvas={addAssetToCanvas} />
                  )}

                  {visibleTasks.length > 0 && (
                    <div className='mt-5'>
                      <h3 className='mb-3 text-sm font-semibold text-white'>本次会话任务</h3>
                      <div className='grid gap-2'>
                        {visibleTasks.slice(0, 4).map((task) => (
                          ...
                          >
                            ...
                            <span className='min-w-0'>
                              <span className='block truncate text-sm font-medium text-white'>{task.model_id}</span>
                              <span className='mt-1 block truncate text-xs text-white/38'>{task.prompt}</span>
                            </span>
                            <span className='font-mono text-xs text-white/42'>{task.cost} 点</span>
                          </button>
                        ))}
                      </div>
                    </div>
            ...
            {view === 'prompts' && (
              <section className='h-full overflow-y-auto p-5 lg:p-8'>
                <div className='mb-6 flex flex-wrap items-end justify-between gap-4'>
                  <div>
                    <p className='text-xs tracking-[0.2em] text-white/35'>提示词库</p>
                    <h1 className='mt-2 text-3xl font-semibold text-white'>提示词库</h1>
                    <p className='mt-2 max-w-2xl text-sm text-white/48'>把可复用的提示词、参数和风格变成下一次生成的起点。</p>
                  </div>
                  <Button className='h-10 rounded-xl bg-white text-black hover:bg-cyan-50' onClick={() => setView('image')}>
                ...
                <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-3'>
                  {templates.map((template) => (
                    <article key={template.id} className='rounded-2xl border border-white/10 bg-white/[0.035] p-4'>
                      <div className='mb-3 flex items-center justify-between gap-2'>
                        <span className='rounded-lg border border-white/10 bg-black/35 px-2 py-1 text-[11px] text-white/48'>
                          {template.kind === 'video' ? '视频' : '图片'} / {modeLabels[template.mode ?? ''] ?? template.style}
                        </span>
                        <Button size='sm' variant='outline' className='h-8 rounded-lg border-white/10 bg-white/[0.035] text-white hover:bg-white/10' onClick={() => applyTemplate(template)}>
                          使用
                        </Button>
                      </div>
                      ...
                      <p className='mt-3 line-clamp-6 text-sm leading-6 text-white/52'>{template.prompt}</p>
                    </article>
                  ))}
                </div>
              </section>
            ...
            {view === 'assets' && (
              <section className='h-full overflow-y-auto p-5 lg:p-8'>
                <div className='mb-6 flex flex-wrap items-end justify-between gap-4'>
                  <div>
                    <p className='text-xs tracking-[0.2em] text-white/35'>素材库</p>
                    <h1 className='mt-2 text-3xl font-semibold text-white'>我的素材</h1>
                    <p className='mt-2 max-w-2xl text-sm text-white/48'>生成结果、参考资产和画布实验会在同一用户端沉淀。</p>
                  </div>
                  <div className='flex gap-2'>
                    <Button variant='outline' className='h-10 rounded-xl border-white/15 bg-white/[0.035] text-white hover:bg-white/10' onClick={refreshGallery}>
                      <RefreshCw data-icon='inline-start' />
                      刷新
                    </Button>
                    <Button className='h-10 rounded-xl bg-white text-black hover:bg-cyan-50' onClick={() => setView('image')}>
                ...
                <AssetGrid tasks={completedTasks} onSendToCanvas={addAssetToCanvas} />
              </section>
            )}
          </main>
        </div>
