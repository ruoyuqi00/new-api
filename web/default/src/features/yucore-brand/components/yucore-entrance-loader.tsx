...
function getLoaderAccentColor(tone: LoaderAccentTone) {
  if (tone === 'amber') {
...
function getLoaderShardColor(tone: LoaderShardTone) {
  if (tone === 'amber') {
...
const loaderPowerOrbitalDots = Array.from({ length: 260 }, (_, index) => {
  ...
  const side = index % 2 === 0 ? -1 : 1
  let x = getLoaderPowerOrbitalX(seedA, angle, radius, side, orbital, sideField)
...
const loaderSphereShards = Array.from(
  { length: LOADER_SHARD_COUNT },
  (_, index) => {
...
export function YucoreEntranceLoader(props: YucoreEntranceLoaderProps) {
  ...
  return (
    ...
    >
      ...
      />
      <div
        className='yucore-loader-power-field absolute inset-0'
        aria-hidden='true'
      >
        {loaderPowerDots.map((dot) => (
          <span
            key={dot.id}
            ...
            style={
              {
                ...
                '--power-x': `${dot.x}%`,
                '--power-y': `${dot.y}%`,
                color: getLoaderAccentColor(dot.tone),
              } as CSSProperties
            }
          />
        ))}
        {loaderPowerOrbitalDots.map((dot) => (
          <span
            key={dot.id}
            className={cn(
      ...
      <div
        className='yucore-loader-far-particle-field absolute inset-0'
        aria-hidden='true'
      >
        {loaderFarFieldDots.map((dot) => (
          <span
            key={dot.id}
            ...
            style={
              {
                ...
                '--dot-y': `${dot.y}%`,
                color: getLoaderAccentColor(dot.tone),
                backgroundColor: getLoaderAccentColor(dot.tone),
              } as CSSProperties
            }
          />
        ))}
      ...
      <div
        className='yucore-loader-sphere absolute left-1/2'
        aria-hidden='true'
      >
              ...
              style={
                {
                  ...
                  '--shard-x': `${shard.x}%`,
                  '--shard-y': `${shard.y}%`,
                  color: getLoaderShardColor(shard.tone),
                } as CSSProperties
              }
            />
          ))}
