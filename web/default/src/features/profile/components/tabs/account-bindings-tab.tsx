...
interface PendingOAuthBinding {
  ...
  state: string
  popup: Window
  lifecycle?: ReturnType<typeof manageOAuthPopupLifecycle>
}
...
export function AccountBindingsTab({
  ...
  const startOAuthBinding = useCallback(
    async (provider: string, buildUrl: (state: string) => string) => {
      ...
      pending.lifecycle = manageOAuthPopupLifecycle(popup, {
        onClosed: () => clearPendingOAuthBinding(pending),
        onTimeout: () => {
          const isCurrent = pendingOAuthBinding.current === pending
          clearPendingOAuthBinding(pending)
          if (!popup.closed) popup.close()
          if (isCurrent) {
            toast.error(t('OAuth binding timed out. Please try again.'))
          }
        },
        timeout: 600_000,
      })
  ...
  useEffect(() => {
    ...
    const handleMessage = async (event: MessageEvent<unknown>) => {
      ...
      if (!pending.lifecycle?.beginExchange()) return
      ...
      try {
        ...
        const response = await api.get(`/api/oauth/${message.provider}`, {
          params,
          signal: pending.lifecycle?.signal,
          skipBusinessError: true,
        })
        if (pendingOAuthBinding.current !== pending) return
      ...
      clearPendingOAuthBinding(pending, false)
      if (success) {
