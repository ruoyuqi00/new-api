...
type GroupRatioVisualEditorProps = {
  ...
  onPricingValidityChange: (valid: boolean) => void
...
function GroupPricingTable({
  ...
  const removeRow = useCallback(
    (id: string) => {
      ...
      const savedGroup = catalog.find((item) => item.name === row.name.trim())
      const ruleSources = getSpecialRuleSources(
        row.name.trim(),
        specialUsableGroups
      )
      const hasDependencies =
        (savedGroup?.active_channel_count ?? 0) > 0 || ruleSources.length > 0
      if (hasDependencies) {
        setPendingRemoval({
          id,
          description: t(
            'This group still has {{channels}} active channels and {{rules}} authorization references. Deleting the pricing row will not delete those dependencies. Continue?',
            {
              channels: savedGroup?.active_channel_count ?? 0,
    ...
    [catalog, emitRows, rows, specialUsableGroups, t]
  )

  const confirmRemoval = useCallback(() => {
    if (!pendingRemoval) return
    emitRows(rows.filter((row) => row._id !== pendingRemoval.id))
    setPendingRemoval(null)
  }, [emitRows, pendingRemoval, rows])
  ...
  useEffect(() => {
    onValidityChange(duplicateNames.length === 0 && !hasEmptyNames)
  }, [duplicateNames.length, hasEmptyNames, onValidityChange])
  ...
  return (
    <Card className={sectionCardClassName}>
        ...
        <div className='space-y-3'>
          <StaticDataTable
            ...
            columns={[
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      updateRow(
                        row._id,
                        'ratio',
                        normalizeGroupRatio(event.target.value)
                      )
                    }
                  />
                ),
                  ...
                  <div className='flex items-center gap-2'>
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        updateRow(row._id, 'public', checked === true)
                      }
              ...
              {
                id: 'routing',
                header: t('Routing coverage'),
                className: 'min-w-48',
                cell: (row) => {
                  if (catalogLoading) {
                    return (
                      <span className='text-muted-foreground text-sm'>
                        {t('Loading routing coverage...')}
                      </span>
                    )
                  }
                  ...
                  const state = getGroupCoverageState(row.name, catalog)
                  const savedGroup = catalog.find(
                    (item) => item.name === row.name.trim()
                  )
                  if (state === 'unsaved') {
                    return (
                      <StatusBadge
                        label={t('Check routing after saving')}
                        variant='neutral'
                        copyable={false}
                      />
                    )
                  ...
                  if (state === 'missing') {
                    return (
                      <div className='space-y-1'>
                        <StatusBadge
                          label={t('No active models')}
                          variant='danger'
                          copyable={false}
                        />
                        <p className='text-destructive text-xs'>
                          {t('API calls to this group will fail.')}
                        </p>
                      </div>
                    )
                  }
                  ...
                  return (
                    <div
                      className='space-y-1'
                      title={savedGroup?.active_models.join(', ')}
                    >
                      <StatusBadge
                        label={t('{{channels}} channels / {{models}} models', {
                          channels: savedGroup?.active_channel_count ?? 0,
                          models: savedGroup?.active_model_count ?? 0,
                        })}
                        variant='success'
                        copyable={false}
                      />
                      <p className='text-muted-foreground max-w-56 truncate text-xs'>
                        {savedGroup?.active_models.join(', ')}
                      </p>
                    </div>
                  )
                },
                ...
                cell: (row) =>
                  row.public ? (
                    ...
                    <span className='text-muted-foreground px-3 text-sm'>
                      {t('Available only through user-group authorization')}
                    </span>
                  ),
              },
              {
      ...
      <ConfirmDialog
        ...
        title={t('Delete')}
        desc={pendingRemoval?.description ?? ''}
