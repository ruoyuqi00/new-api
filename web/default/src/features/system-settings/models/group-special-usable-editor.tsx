        ...
        <div className='flex items-center justify-between p-3'>
          <div className='flex items-center gap-2'>
            ...
            <span className='font-semibold'>
              {t('Source user group')}: {props.groupName}
            </span>
        ...
        <CollapsibleContent>
          <div className='space-y-2 border-t p-3'>
            {props.items.map((rule) => {
              const targetName = rule.targetGroup.trim()
              const target = props.catalog.find(
                (item) => item.name === targetName
              )
              const missingTarget =
                props.catalogReady && targetName !== '' && !target
              const missingRouting =
                props.catalogReady &&
                rule.op !== OP_REMOVE &&
                target?.active_model_count === 0
              ...
              return (
                ...
                >
                  <div className='grid gap-2 lg:grid-cols-[130px_minmax(10rem,1fr)_minmax(10rem,1fr)_2rem]'>
                    <Select
                      items={[
                        {
                          value: OP_ADD,
                    ...
                    >
                      <SelectTrigger aria-label={t('Authorization operation')}>
                        <SelectValue>
                          <StatusBadge
                    ...
                    <Input
                      ...
                      placeholder={t('Target private group')}
                      aria-label={t('Target private group')}
                      aria-invalid={missingTarget || missingRouting}
                      onChange={(e) =>
                    ...
                    {rule.op !== OP_REMOVE ? (
                      <Input
                        ...
                        placeholder={t('Authorization description')}
                        aria-label={t('Authorization description')}
                        onChange={(e) =>
                          props.onUpdate(
                            rule._id,
                            'description',
                  ...
                  {missingTarget && (
                    <p className='text-destructive text-xs'>
                      {t(
                        'This target does not exist in pricing groups. Save a matching pricing group first.'
                      )}
                    </p>
                  )}
                  {missingRouting && (
                    <p className='text-destructive text-xs'>
                      {t(
                        'This target has no active models. Authorization will be visible, but API calls will fail.'
                      )}
                    </p>
                  )}
                </div>
  ...
  return (
    <Card className={sectionCardClassName}>
      <CardHeader className={sectionHeaderClassName}>
        <CardTitle>{t('User group authorization')}</CardTitle>
        <CardDescription>
          {t(
            'Grant a source user group access to private target groups. These rules change visibility and selection only.'
          )}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className='space-y-3'>
          <div className='border-info/30 bg-info/5 text-muted-foreground rounded-md border px-3 py-2 text-sm leading-6'>
            {t(
              '+: grants access to a private group. It does not create channels, copy models, or change the group ratio.'
            )}
          </div>
          {grouped.length === 0 ? (
          ...
          <div className='flex items-center justify-center gap-2 pt-2'>
            <Input
              className='w-[200px]'
              value={newGroupName}
              placeholder={t('Source user group')}
