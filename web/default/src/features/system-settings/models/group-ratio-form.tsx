...
import { getGroupCatalog } from '../api'
import {
  SettingsForm,
...
export const GroupRatioForm = memo(function GroupRatioForm({
  ...
  const groupCatalogQuery = useQuery({
    queryKey: ['group-catalog'],
    queryFn: getGroupCatalog,
    staleTime: 60 * 1000,
  })
  const groupCatalog = groupCatalogQuery.data?.success
    ? groupCatalogQuery.data.data
    : []
  const refetchGroupCatalog = groupCatalogQuery.refetch
  const catalogReady =
    !groupCatalogQuery.isPending &&
    !groupCatalogQuery.isError &&
    groupCatalogQuery.data?.success === true
  ...
  const toggleEditMode = useCallback(() => {
    ...
    if (nextMode === 'json') setPricingValid(true)
  }, [editMode])

  const handleSave = useCallback(
    async (values: GroupFormValues) => {
      await onSave(values)
      await refetchGroupCatalog()
    },
    [onSave, refetchGroupCatalog]
  )
      ...
      <Form {...form}>
        <SettingsPageActionsPortal>
          <Button
            ...
            disabled={isSaving || !pricingValid}
          >
        ...
        {editMode === 'visual' ? (
          <div className='space-y-6'>
            <GroupRatioVisualEditor
              ...
              catalogLoading={groupCatalogQuery.isPending}
              catalogUnavailable={!groupCatalogQuery.isPending && !catalogReady}
          ...
          <section className='space-y-2'>
            ...
            <div className='text-muted-foreground space-y-2 text-sm leading-6'>
              ...
              <p>
                ...
                {': '}
                {t(
                  'Public groups are visible to ordinary users. Private groups require matching user-group membership or an explicit authorization rule.'
                )}
              </p>
            </div>
          </section>
            ...
            <AccordionItem value='groups'>
              ...
              <AccordionContent className='space-y-3'>
                <p className='text-muted-foreground text-sm leading-6'>
                  {t(
                    'Use the pricing group table to manage ratios, public visibility, and saved routing coverage.'
                  )}
                </p>
                <GuideCodeBlock>
                  {`${t('Group name')}   ${t('Ratio')}   ${t('Visibility')}   ${t('Description')}
standard     1.0     ${t('Public')}       ${t('Standard price')}
premium      0.5     ${t('Public')}       ${t('Premium plan, half price')}
downstream   0.5     ${t('Private')}      ${t('Available only through user-group authorization')}`}
                </GuideCodeBlock>
                <p className='text-muted-foreground text-sm leading-6'>
                  {t(
                    'A private group becomes usable only when the user is authorized and the group has active channel and model routing coverage.'
                  )}
                </p>
              </AccordionContent>
            </AccordionItem>
    ...
    "premium": 0.3
            ...
            <AccordionItem value='usable'>
              ...
              <AccordionContent className='space-y-3'>
                <p className='text-muted-foreground text-sm leading-6'>
                  {t(
                    'User-group authorization rules grant or remove access to target token groups for a source user group.'
                  )}
                </p>
                <GuideCodeBlock>{`{
  "vip": {
    ...
    "special": "${t('Special group')}"
                ...
                <p className='text-muted-foreground text-sm leading-6'>
                  {t(
                    'Use +: to grant visibility and selection, or -: to remove access. Authorization does not create channels, copy models, or change ratios.'
                  )}
                </p>
              </AccordionContent>
            </AccordionItem>
