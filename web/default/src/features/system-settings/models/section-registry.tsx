...
import { YucoreMediaSettingsCard } from './yucore-media-settings-card'
...
const yucoreMediaAdapters = ['mock', 'openai-compatible', 'uag-proxy'] as const

function normalizeYucoreMediaAdapter(value: string) {
  return yucoreMediaAdapters.includes(value as (typeof yucoreMediaAdapters)[number])
    ? (value as (typeof yucoreMediaAdapters)[number])
    : 'mock'
}
...
const MODELS_SECTIONS = [
  ...
  {
    id: 'yucore-studio',
    titleKey: 'YuCore Studio',
    build: (settings: ModelSettings) => (
      <YucoreMediaSettingsCard
        defaultValues={{
          'yucore_media.adapter': normalizeYucoreMediaAdapter(settings['yucore_media.adapter']),
          'yucore_media.base_url': settings['yucore_media.base_url'] ?? '',
          'yucore_media.timeout_seconds':
            settings['yucore_media.timeout_seconds'] ?? 90,
          'yucore_media.require_real_assets':
            settings['yucore_media.require_real_assets'] ?? false,
          'yucore_media.uag_model_map':
            settings['yucore_media.uag_model_map'] ?? '',
          'yucore_media.uag_allowed_providers':
            settings['yucore_media.uag_allowed_providers'] ?? '',
          'yucore_media.uag_allowed_models':
            settings['yucore_media.uag_allowed_models'] ?? '',
          'yucore_media.upstream_verified':
            settings['yucore_media.upstream_verified'] ?? false,
        }}
      />
    ),
  },
