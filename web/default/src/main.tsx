    ...
    onError: (error, query) => {
      if (error instanceof AxiosError) {
        if (error.response?.status === 500) {
          if (shouldShowGlobalQueryErrorToast(query.meta)) {
            toast.error(i18next.t('Internal Server Error!'))
          }
          if (shouldNavigateToServerError(error.response.status, query.meta)) {
...
if (!rootElement.innerHTML) {
  ...
  const bootFallbackTimer = window.setTimeout(finishBoot, 8000)
