...
export function captureAuthRequestSnapshot(): AuthRequestSnapshot {
  const auth = useAuthStore.getState().auth
  return Object.freeze({
    epoch: authEpoch,
    sessionSID: auth.session?.sid,
    userID: auth.user?.id,
  })
}

export function isAuthRequestSnapshotCurrent(
  snapshot: AuthRequestSnapshot
): boolean {
  const auth = useAuthStore.getState().auth
  return (
    snapshot.epoch === authEpoch &&
    snapshot.sessionSID === auth.session?.sid &&
    snapshot.userID === auth.user?.id
  )
}

export function isAuthRequestSessionCurrent(
  snapshot: AuthRequestSnapshot
): boolean {
  if (snapshot.sessionSID === undefined || snapshot.userID === undefined) {
    return false
  }
  const auth = useAuthStore.getState().auth
  return (
