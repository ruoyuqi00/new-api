...
describe('authentication session coordination', () => {
  ...
  test('a refresh response cannot restore credentials after a newer auth operation', async () => {
    const bundleB: AuthBundle = {
      ...bundle,
      access_token: 'account-b-token',
      user: { id: 84, username: 'account-b', role: 1 },
    ...
    const runtime: AuthRefreshRuntime = {
      ...
      markTransient: () => undefined,
      wait: async () => undefined,
      isCurrent: () => isAuthRequestSnapshotCurrent(snapshot),
    }
    ...
    assert.equal(useAuthStore.getState().auth.session?.sid, 'session-b')
    assert.equal(useAuthStore.getState().auth.accessToken, 'account-b-token')
  })
