...
import { YUCORE_BRAND_NAME, YucoreBrandMark } from '@/features/yucore-brand'
...
export function SystemBrand(props: SystemBrandProps) {
  ...
  const name = props.defaultName || YUCORE_BRAND_NAME
  const version =
  ...
  if (variant === 'inline') {
    return (
      ...
      >
        <YucoreBrandMark compact className='[&>span:first-child]:size-5' />
        <span className='max-w-[12rem] truncate'>{YUCORE_BRAND_NAME}</span>
      </Link>
    )
  }
