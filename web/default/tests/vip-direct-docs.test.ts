...
describe('VIP direct API documentation', () => {
  ...
  it('documents current text, image, and video boundaries', () => {
    ...
    expect(docs).toContain('[模型广场](/pricing)')
    expect(docs).not.toContain('0.02619')
    expect(docs).not.toContain('0.0414')
    expect(docs).not.toContain('0.0594')
    expect(docs).not.toContain('0.0774')
    expect(docs).not.toContain('0.9936')
    expect(docs).not.toContain('2.0016')
