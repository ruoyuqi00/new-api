<template>
  <BaseDialog
    :show="show"
    :title="t('admin.accounts.providerAdaptersTitle')"
    width="extra-wide"
    close-on-click-outside
    @close="emit('close')"
  >
    <div class="space-y-4">
      <div class="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-700 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-200">
      ...
      <div class="rounded-xl border border-gray-200 dark:border-dark-700">
        ...
        <pre class="max-h-[52vh] overflow-auto rounded-b-xl bg-gray-50 p-4 text-xs text-gray-800 dark:bg-dark-900 dark:text-dark-100">{{ activeRaw }}</pre>
      </div>
    </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        <button class="btn btn-primary" type="button" :disabled="loadingAny" @click="loadAll">
          {{ t('admin.accounts.providerAdaptersRefresh') }}
        </button>
      </div>
    </template>
...
type EndpointKey = 'windsurfHealth' | 'windsurfAccounts' | 'kiroCredentials'
...
const activeKey = ref<EndpointKey>('windsurfAccounts')
...
watch(
  ...
  (open) => {
    if (open) {
      activeKey.value = 'windsurfAccounts'
      loadAll()
    }
  }
)
