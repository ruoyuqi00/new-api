...
func TestRecordRelaySampleRecordsTextGroupWhenMonitoringToggleIsUnavailable(t *testing.T) {
	server := miniredis.RunT(t)
	originalRedisEnabled, originalRedis := common.RedisEnabled, common.RDB
	originalMonitoring := ratio_setting.AvailabilityMonitoring2JSONString()
	common.RedisEnabled = true
	common.RDB = redis.NewClient(&redis.Options{Addr: server.Addr()})
	require.NoError(t, ratio_setting.UpdateAvailabilityMonitoringByJSONString(`{}`))
	hotBuckets = sync.Map{}
	t.Cleanup(func() {
		hotBuckets = sync.Map{}
		common.RedisEnabled = originalRedisEnabled
