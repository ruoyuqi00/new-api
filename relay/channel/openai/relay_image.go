...
func OpenaiImageHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if !oaiImageResponseHasSignal(imageResp) {
		return nil, types.NewOpenAIError(fmt.Errorf("empty OpenAI-compatible image response"), types.ErrorCodeEmptyResponse, http.StatusBadGateway)
	}
	if converted, err := convertOpenAIImageURLsToB64JSON(c, info, &imageResp); err != nil {
	...
	// 写入新的 response body
	updateOpenAIImageCount(info, int64(len(imageResp.Data)))
...
func oaiImageResponseHasSignal(resp dto.ImageResponse) bool {
...
func OpenaiImageStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if info != nil && info.StreamStatus != nil {
		...
		// A client abort makes the observed count incomplete, so it may raise
		// the charge but must never lower the requested count.
...
func OpenaiImageJSONAsStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if _, err := convertOpenAIImageURLsToB64JSON(c, info, &imageResp); err != nil {
		return nil, types.NewOpenAIError(err, types.ErrorCodeBadResponse, http.StatusBadGateway)
	}
	updateOpenAIImageCount(info, int64(len(imageResp.Data)))
	normalizeOpenAIUsage(&usageResp.Usage)
...
func convertOpenAIImageURLsToB64JSON(c *gin.Context, info *relaycommon.RelayInfo, imageResp *dto.ImageResponse) (bool, error) {
	if !openAIImageRequestWantsB64JSON(info) || imageResp == nil {
		return false, nil
	}

	converted := false
	for i := range imageResp.Data {
		image := &imageResp.Data[i]
		if strings.TrimSpace(image.B64Json) != "" || strings.TrimSpace(image.Url) == "" {
			continue
		}
		b64, _, err := service.GetBase64Data(c, types.NewURLFileSource(image.Url), "openai_image_response")
		if err != nil {
			return converted, fmt.Errorf("failed to convert upstream image url to b64_json: %w", err)
		}
		image.B64Json = b64
		image.Url = ""
		converted = true
...
func openAIImageRequestWantsB64JSON(info *relaycommon.RelayInfo) bool {
