...
func TestOaiResponsesCompactionHandlerEstimatedEmptyOutputUsesBillingPlaceholder(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses/compact", nil)
	...
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Body:       io.NopCloser(strings.NewReader(`{"id":"cmp-1","object":"response.compaction","output":[]}`)),
	}
