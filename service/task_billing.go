...
func LogTaskConsumption(c *gin.Context, info *relaycommon.RelayInfo) {
	...
	// 支持任务仅按次计费
	if model.YucoreMediaModelUsesPerCallPricing(info.OriginModelName) {
...
func RefundTaskQuota(ctx context.Context, task *model.Task, reason string) bool {
	...
	if task.ID != 0 {
		if err := task.UpdateQuota(); err != nil {
			logger.LogError(ctx, fmt.Sprintf("任务退款后回写 quota 失败 task %s: %s", task.TaskID, err.Error()))
		}
	}
...
func RecalculateTaskQuotaByTokens(ctx context.Context, task *model.Task, totalTokens int) {
	...
	actualQuota, clamp := taskTokenRecalculatedQuotaChecked(totalTokens, modelRatio, finalGroupRatio, otherMultiplier)

	reason := fmt.Sprintf("token重算：tokens=%d, modelRatio=%.2f, groupRatio=%.2f, otherMultiplier=%.4f", totalTokens, modelRatio, finalGroupRatio, otherMultiplier)
	RecalculateTaskQuota(ctx, task, actualQuota, reason, clamp.WithOp("task_token_recalculation"))
}

func taskTokenRecalculatedQuota(totalTokens int, modelRatio, groupRatio, otherMultiplier float64) int {
	quota, _ := taskTokenRecalculatedQuotaChecked(totalTokens, modelRatio, groupRatio, otherMultiplier)
	return quota
}

func taskTokenRecalculatedQuotaChecked(totalTokens int, modelRatio, groupRatio, otherMultiplier float64) (int, *common.QuotaClamp) {
	return common.QuotaFromFloatChecked(float64(totalTokens) * modelRatio * groupRatio * otherMultiplier)
}

func firstQuotaClamp(clamps ...*common.QuotaClamp) *common.QuotaClamp {
