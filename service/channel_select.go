...
func (p *RetryParam) PrepareForRetry(channelID int, providerAccountSelected bool) {
	if providerAccountSelected {
		p.providerChannelID = channelID
	} else {
		p.providerChannelID = 0
		p.SkipChannelID(channelID)
	}
	p.ResetRetryNextTry()
}

// TakeProviderRetryChannelID returns the channel whose account pool should be
// retried before normal channel selection, then consumes that preference.
func (p *RetryParam) TakeProviderRetryChannelID() int {
	channelID := p.providerChannelID
	p.providerChannelID = 0
	return channelID
}

func (p *RetryParam) SkipChannelID(channelID int) {
	if channelID <= 0 {
		return
	}
	if p.skippedChannelIDs == nil {
		p.skippedChannelIDs = make(map[int]struct{})
	}
	p.skippedChannelIDs[channelID] = struct{}{}
}
...
func CacheGetRandomSatisfiedChannel(param *RetryParam) (*model.Channel, string, error) {
	...
	if param.TokenGroup == "auto" {
		...
		channel, err = model.GetRandomSatisfiedChannelWithOptions(param.TokenGroup, param.ModelName, param.GetRetry(), param.RequestPath, param.ChannelSelectionOptions())
		if err != nil {
