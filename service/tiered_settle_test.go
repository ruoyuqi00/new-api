...
func TestPrepareTieredBillingForSelectedGroupUpdatesReservation(t *testing.T) {
	...
	relayInfo := &relaycommon.RelayInfo{
		...
		PriceData: projecttypes.PriceData{
			GroupRatioInfo: projecttypes.GroupRatioInfo{GroupRatio: 0.20},
		},
	}
...
func TestPrepareTieredBillingForSelectedGroupPaidToFreeKeepsFreeModelFalse(t *testing.T) {
	...
	relayInfo := &relaycommon.RelayInfo{
		...
		PriceData: projecttypes.PriceData{
			GroupRatioInfo: projecttypes.GroupRatioInfo{GroupRatio: 0},
		},
	}
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			relayInfo := &relaycommon.RelayInfo{
				...
				PriceData: projecttypes.PriceData{
					GroupRatioInfo: projecttypes.GroupRatioInfo{GroupRatio: test.finalGroupRatio},
				},
			}
			...
			assert.Equal(t, test.finalGroupRatio, relayInfo.TieredBillingSnapshot.GroupRatio)
			assert.Equal(t, test.wantQuota, relayInfo.TieredBillingSnapshot.EstimatedQuotaAfterGroup)
		})
	}
}
...
func TestBuildTieredTokenParams_GPT_WithNativeCacheWrite(t *testing.T) {
	...
	if params.P != 0 || params.CR != 700 || params.CC != 900 {
		t.Fatalf("params = %+v, want P=0 CR=700 CC=900", params)
	}
}
