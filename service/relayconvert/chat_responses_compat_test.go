...
func TestResponsesStreamEventToChatChunksReusesToolByCallID(t *testing.T) {
	state := newTestResponsesStreamState()
	firstOutputIndex := 0
	secondOutputIndex := 1
...
func TestResponsesStreamEventToChatChunksDoesNotDuplicateTerminalToolCall(t *testing.T) {
	tests := []struct {
		name         string
		terminalType string
		terminalItem dto.ResponsesOutput
	}{
		{
			name:         "same item ID",
			terminalType: responsesEventCompleted,
			terminalItem: dto.ResponsesOutput{
				Type:      responsesOutputTypeFunctionCall,
				ID:        "fc_item_1",
				Name:      "lookup",
		...
		{
			name:         "same call ID",
			terminalType: responsesEventDone,
			terminalItem: dto.ResponsesOutput{
				Type:      responsesOutputTypeFunctionCall,
				ID:        "fc_terminal_1",
				CallId:    "call_1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			chunks = append(chunks, mustStreamChunks(t, state, &dto.ResponsesStreamResponse{
				Type: tt.terminalType,
				Response: &dto.OpenAIResponsesResponse{
					Status: []byte(`"completed"`),
					Output: []dto.ResponsesOutput{tt.terminalItem},
				},
			})...)
			...
			require.NotNil(t, chunks[len(chunks)-1].Choices[0].FinishReason)
			assert.Equal(t, "tool_calls", *chunks[len(chunks)-1].Choices[0].FinishReason)
		})
	}
}
