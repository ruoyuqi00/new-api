...
func registerChannelRoutes(apiRouter *gin.RouterGroup) {
	...
	accountPoolRoute := apiRouter.Group("/account_pool")
	accountPoolRoute.Use(middleware.AdminAuth())
	accountPoolRoute.GET("/", middleware.RequirePermission(authz.ChannelRead), controller.ListAccountPools)
	accountPoolRoute.GET("/channels", middleware.RequirePermission(authz.ChannelRead), controller.ListAccountPoolChannels)
	accountPoolRoute.GET("/accounts", middleware.RequirePermission(authz.ChannelRead), controller.ListProviderAccounts)
	accountPoolRoute.GET("/accounts/:id/usage", middleware.RequirePermission(authz.ChannelRead), controller.GetProviderAccountUsage)
	accountPoolRoute.POST("/accounts/usage/refresh", middleware.RequirePermission(authz.ChannelOperate), controller.RefreshProviderAccountUsages)
	accountPoolRoute.POST("/accounts/assign", middleware.RequirePermission(authz.ChannelWrite), controller.AssignProviderAccounts)
	accountPoolRoute.POST("/accounts/status", middleware.RequirePermission(authz.ChannelOperate), controller.UpdateProviderAccountStatuses)
	accountPoolRoute.POST("/accounts/delete", middleware.RequirePermission(authz.ChannelSensitiveWrite), controller.DeleteProviderAccounts)
	accountPoolRoute.PUT("/accounts/:id/routing", middleware.RequirePermission(authz.ChannelWrite), controller.UpdateProviderAccountRouting)
	accountPoolRoute.POST("/accounts/import", middleware.RequirePermission(authz.ChannelSensitiveWrite), controller.ImportProviderAccounts)
	accountPoolRoute.POST("/grok/oauth/authorize", middleware.RequirePermission(authz.ChannelSensitiveWrite), controller.GenerateAccountPoolGrokOAuthAuthorization)
	accountPoolRoute.POST("/grok/oauth/exchange", middleware.RequirePermission(authz.ChannelSensitiveWrite), controller.ExchangeAccountPoolGrokOAuthAuthorization)
	accountPoolRoute.GET("/:id/models", middleware.RequirePermission(authz.ChannelOperate), controller.GetAccountPoolModels)
	accountPoolRoute.GET("/:id", middleware.RequirePermission(authz.ChannelRead), controller.GetAccountPool)
	accountPoolRoute.POST("/", middleware.RequirePermission(authz.ChannelSensitiveWrite), controller.CreateAccountPool)
	accountPoolRoute.PUT("/:id", middleware.RequirePermission(authz.ChannelSensitiveWrite), controller.UpdateAccountPool)
	accountPoolRoute.DELETE("/:id", middleware.RequirePermission(authz.ChannelSensitiveWrite), controller.DeleteAccountPool)
	accountPoolRoute.DELETE("/accounts/:id", middleware.RequirePermission(authz.ChannelSensitiveWrite), controller.DeleteProviderAccount)
	accountPoolRoute.POST("/accounts/:id/credential", middleware.RootAuth(), middleware.CriticalRateLimit(), middleware.DisableCache(), middleware.SecureVerificationRequired(), controller.GetProviderAccountCredential)
}
