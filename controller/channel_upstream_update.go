...
const (
	...
	codexModelDiscoveryClientVersion                      = "0.144.1"
	codexModelDiscoveryUserAgent                          = "codex_cli_rs/0.144.1 (Mac OS 26.3.1; arm64) iTerm.app/3.6.9"
)

type codexUpstreamModelsResponse struct {
	Models []struct {
...
func getFetchModelsResponseBody(method string, requestURL string, channel *model.Channel, headers http.Header) ([]byte, error) {
	...
	if response.StatusCode != http.StatusOK {
		return nil, &upstreamHTTPStatusError{StatusCode: response.StatusCode}
	}
	return io.ReadAll(response.Body)
}

func fetchChannelUpstreamModelIDs(channel *model.Channel) ([]string, error) {
	poolIds, err := model.GetEnabledAccountPoolIdsByChannel(channel.Id, channel.Type)
	if err != nil {
		return nil, fmt.Errorf("获取渠道绑定账号池失败: %w", err)
	}
	if len(poolIds) == 0 {
		return fetchCredentialUpstreamModelIDs(channel)
	}

	var commonModels []string
	for i, poolId := range poolIds {
		discovery, err := discoverAccountPoolModels(poolId, channel.Id)
		if err != nil {
			return nil, fmt.Errorf("探测账号池 #%d 模型失败: %w", poolId, err)
		}
		if !discovery.Complete {
			return nil, fmt.Errorf("账号池 #%d 仅成功探测 %d/%d 个启用账号", poolId, discovery.SucceededAccounts, discovery.TotalAccounts)
		}
		if i == 0 {
			commonModels = discovery.CommonModels
			continue
		}
		commonModels = intersectModelNames(commonModels, discovery.CommonModels)
	}
	return normalizeModelNames(commonModels), nil
}

func fetchCredentialUpstreamModelIDs(channel *model.Channel) ([]string, error) {
	baseURL := constant.ChannelBaseURLs[channel.Type]
	if channel.GetBaseURL() != "" {
		baseURL = channel.GetBaseURL()
	}
	if channel.Type == constant.ChannelTypeXai && strings.HasPrefix(strings.TrimSpace(channel.Key), "{") {
		if err := service.ProbeGrokOAuthCredential(context.Background(), baseURL, channel.Key); err != nil {
			return nil, err
		}
		return normalizeModelNames(xai.ModelList), nil
	}
	...
	case constant.ChannelTypeCodex:
		// Keep model discovery aligned with the current CLIProxyAPI Codex catalog
		// request so upstream version gates return the real account model list.
		url = fmt.Sprintf("%s/backend-api/codex/models?client_version=%s", strings.TrimRight(baseURL, "/"), codexModelDiscoveryClientVersion)
	case constant.ChannelTypeXai:
		if strings.HasPrefix(strings.TrimSpace(channel.Key), "{") {
			if strings.TrimSpace(baseURL) == "" || strings.EqualFold(strings.TrimRight(baseURL, "/"), "https://api.x.ai") || strings.EqualFold(strings.TrimRight(baseURL, "/"), "https://api.x.ai/v1") {
	...
	if channel.Type == constant.ChannelTypeCodex {
		var result codexUpstreamModelsResponse
		if err := common.Unmarshal(body, &result); err != nil {
			return nil, err
		}
		ids := make([]string, 0, len(result.Models))
		for _, item := range result.Models {
			if item.Visibility != "" && item.Visibility != "list" {
				continue
			}
