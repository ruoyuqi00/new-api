...
func buildFetchModelsHeaders(channel *model.Channel, key string) (http.Header, error) {
	...
	case constant.ChannelTypeCodex:
		...
		headers.Set("User-Agent", codexModelDiscoveryUserAgent)
	case constant.ChannelTypeXai:
		...
		if strings.HasPrefix(strings.TrimSpace(key), "{") {
			...
			if err := common.UnmarshalJsonStr(key, &oauthCredential); err != nil {
				return nil, fmt.Errorf("xAI OAuth credential is not valid JSON")
			}
			accessToken := strings.TrimSpace(oauthCredential.AccessToken)
			if accessToken == "" {
				return nil, fmt.Errorf("xAI OAuth credential requires access_token")
			}
			headers.Set("Authorization", "Bearer "+accessToken)
			headers.Set("X-XAI-Token-Auth", "xai-grok-cli")
			headers.Set("x-grok-client-version", constant.GrokCLIClientVersion)
			headers.Set("User-Agent", constant.GrokCLIUserAgent)
		}
	default:
		headers = GetAuthHeader(key)
	}
