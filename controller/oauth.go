...
const (
	...
	oauthLoginInitiatorCookiePrefix     = "oauth_login_initiator_"
	oauthLoginInitiatorSecretLength     = 64
	oauthLoginInitiatorHashKeyNamespace = "oauth-login-initiator-v1:"
)
...
type oauthFlowPayload struct {
	...
	LoginInitiatorHash string `json:"login_initiator_hash,omitempty"`
}

func oauthLoginInitiatorCookieName(flowToken string) string {
	return oauthLoginInitiatorCookiePrefix + flowToken
}

func oauthLoginInitiatorHash(secret string) string {
	return common.GenerateHMACWithKey([]byte(oauthLoginInitiatorHashKeyNamespace+common.SessionSecret), secret)
}

func writeOAuthLoginInitiatorCookie(c *gin.Context, flowToken, secret string, expiresAt time.Time) {
	http.SetCookie(c.Writer, &http.Cookie{
		Name:     oauthLoginInitiatorCookieName(flowToken),
		Value:    secret,
		Path:     "/api/oauth",
...
func clearOAuthLoginInitiatorCookie(c *gin.Context, flowToken string) {
	http.SetCookie(c.Writer, &http.Cookie{
		Name:     oauthLoginInitiatorCookieName(flowToken),
		Value:    "",
		Path:     "/api/oauth",
...
func GenerateOAuthCode(c *gin.Context) {
	...
	if request.Intent == model.AuthFlowIntentLogin {
		generatedSecret, err := common.GenerateRandomCharsKey(oauthLoginInitiatorSecretLength)
		if err != nil {
			common.ApiError(c, err)
			return
		}
		initiatorSecret = generatedSecret
		flowPayload.LoginInitiatorHash = oauthLoginInitiatorHash(initiatorSecret)
	}
	...
	if request.Intent == model.AuthFlowIntentLogin {
		writeOAuthLoginInitiatorCookie(c, state, initiatorSecret, expiresAt)
	}
...
func HandleOAuth(c *gin.Context) {
	...
	if pendingFlow.Intent == model.AuthFlowIntentLogin {
		...
		initiatorSecret, err := c.Cookie(oauthLoginInitiatorCookieName(state))
		if err != nil || loginPayload.LoginInitiatorHash == "" ||
			subtle.ConstantTimeCompare([]byte(oauthLoginInitiatorHash(initiatorSecret)), []byte(loginPayload.LoginInitiatorHash)) != 1 {
			c.JSON(http.StatusForbidden, gin.H{
				"success": false,
				"message": i18n.T(c, i18n.MsgOAuthStateInvalid),
			})
	...
	// 3. Handle terminal errors from a provider that may have been disabled
	// after the authorization request began.
	...
	if errorCode != "" {
		...
		if pendingFlow.Intent == model.AuthFlowIntentLogin {
			clearOAuthLoginInitiatorCookie(c, state)
		}
	...
	// 4. Check if provider is enabled before making provider requests.
	...
	user, err := findOrCreateOAuthUser(c, provider, oauthUser, loginPayload.AffiliateCode)
	if err != nil {
	...
	// 9. Setup login
	setupLoginAtAuthVersionWithSuccess(user, 0, c, func() {
		clearOAuthLoginInitiatorCookie(c, state)
	})
}
