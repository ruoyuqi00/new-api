...
func testChannel(ctx context.Context, channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool, imageSize string) testResult {
	...
	tik := time.Now()
	if strings.TrimSpace(endpointType) == string(constant.EndpointTypeOpenAIVideo) {
		return testResult{
			localErr: errors.New("OpenAI video channel test is not supported"),
		}
	}
	...
	if imageRequest, ok := request.(*dto.ImageRequest); ok && !isStream {
		if dimensionErr := validateImageTestResponseDimensions(respBody, imageRequest.Size); dimensionErr != nil {
			return testResult{
				context:     c,
				localErr:    dimensionErr,
				newAPIError: types.NewOpenAIError(dimensionErr, types.ErrorCodeBadResponseBody, http.StatusInternalServerError),
			}
		}
	}
	...
	model.RecordConsumeLog(c, testUserID, model.RecordConsumeLogParams{
		...
		ActualResponseModel: info.ActualResponseModel,
		TokenName:           "模型测试",
		Quota:               quota,
		Content:             "模型测试",
...
func validateImageTestResponseDimensions(respBody []byte, requestedSize string) error {
	widthText, heightText, ok := strings.Cut(strings.ToLower(strings.TrimSpace(requestedSize)), "x")
	if !ok {
		return fmt.Errorf("invalid requested image size %q", requestedSize)
	}
	requestedWidth, widthErr := strconv.Atoi(strings.TrimSpace(widthText))
	requestedHeight, heightErr := strconv.Atoi(strings.TrimSpace(heightText))
	if widthErr != nil || heightErr != nil || requestedWidth <= 0 || requestedHeight <= 0 {
		return fmt.Errorf("invalid requested image size %q", requestedSize)
	}

	var imageResponse dto.ImageResponse
	if err := common.Unmarshal(respBody, &imageResponse); err != nil {
		return fmt.Errorf("decode image test response: %w", err)
	}
	...
	if strings.TrimSpace(imageData.B64Json) != "" {
		config, _, _, err := service.DecodeBase64ImageData(imageData.B64Json)
		if err != nil {
			return fmt.Errorf("decode generated image dimensions: %w", err)
		}
		actualWidth, actualHeight = config.Width, config.Height
	} else if strings.TrimSpace(imageData.Url) != "" {
		config, _, err := service.DecodeUrlImageData(imageData.Url)
		if err != nil {
			return fmt.Errorf("download generated image dimensions: %w", err)
		}
		actualWidth, actualHeight = config.Width, config.Height
	} else {
		return errors.New("image test response contains no image data")
	}

	// Providers may return nearby canonical dimensions. Require at least 75%
	// of each requested edge so a fixed 1K result cannot pass a 2K/4K test.
	if actualWidth*4 < requestedWidth*3 || actualHeight*4 < requestedHeight*3 {
		return fmt.Errorf(
			"requested image size %dx%d, but upstream returned %dx%d",
			requestedWidth,
			requestedHeight,
			actualWidth,
			actualHeight,
...
func buildTestRequest(model string, endpointType string, channel *model.Channel, isStream bool, imageSize string) dto.Request {
...
func TestChannel(c *gin.Context) {
	...
	result := testChannel(requestCtx, channel, testUserID, testModel, endpointType, isStream, imageSize)
