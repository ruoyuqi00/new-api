							...
							if usingGroup == "auto" {
								...
								for _, g := range autoGroups {
									if model.IsChannelEnabledForGroupModel(g, modelRequest.Model, preferred.Id) {
										if !model.ChannelProtocolAffinityAllowed(preferred, g, modelRequest.Model, c.Request.URL.Path) {
											affinityRequestIncompatible = true
											break
										}
								...
								if !model.ChannelProtocolAffinityAllowed(preferred, usingGroup, modelRequest.Model, c.Request.URL.Path) {
									affinityRequestIncompatible = true
								} else if service.IsChannelPoolTemporarilyUnavailableWithContext(c, preferred, usingGroup, modelRequest.Model) {
									affinityTemporarilyUnavailable = true
								} else {
									channel = preferred
...
func SetupContextForSelectedChannel(c *gin.Context, channel *model.Channel, modelName string) *types.NewAPIError {
	...
	accountRoute, accountPoolBound, accountError := service.ResolveChannelProviderAccount(c, channel)
	if accountError != nil {
		return accountError
	}
	key := ""
	if accountPoolBound {
		key = accountRoute.Credential
	...
	if accountPoolBound {
		if accountRoute.BaseURL != "" {
			baseURL = accountRoute.BaseURL
		}
		if accountRoute.ModelMapping != "" {
			modelMapping = accountRoute.ModelMapping
		}
	}
	if channel.Type == constant.ChannelTypeXai && strings.HasPrefix(strings.TrimSpace(key), "{") &&
		(baseURL == "" || strings.EqualFold(strings.TrimRight(baseURL, "/"), "https://api.x.ai") || strings.EqualFold(strings.TrimRight(baseURL, "/"), "https://api.x.ai/v1")) {
