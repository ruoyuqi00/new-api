...
func TestUserAuthAllowsOpaqueDottedPAT(t *testing.T) {
	...
	router.GET("/protected", UserAuth(), func(c *gin.Context) {
		_, hasSessionIdentity := GetSessionAuthIdentity(c)
		c.JSON(http.StatusOK, gin.H{"id": c.GetInt("id"), "has_session_identity": hasSessionIdentity})
	})
	...
	var body struct {
		ID                 int  `json:"id"`
		HasSessionIdentity bool `json:"has_session_identity"`
	}
	require.NoError(t, common.Unmarshal(response.Body.Bytes(), &body))
	assert.Equal(t, user.Id, body.ID)
	assert.False(t, body.HasSessionIdentity)
}
